...
*hostcall* mechanism. This is meant for "unlikely" paths in device code (reporting,
logging, loading data on demand, error handling), not as a bulk data path: a call costs a
few microseconds and goes through a small packet per thread.
...
- **Asynchronous calls** never send a result back to the device.
