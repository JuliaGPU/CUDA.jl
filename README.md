...
The GPU CI agents on Buildkite are a shared resource, so avoid running more than necessary.
Draft pull requests only test the newest Julia release and nightly; everything else runs
once a pull request is marked ready for review. The steps to run can also be selected with
tags in the first line of the last commit's message:

  - `[only X]`: run only the listed steps, e.g. `[only nightly]` or `[only julia, special]`
    (this also works on draft pull requests)
  - `[skip X]`: don't run the listed steps, e.g. `[skip benchmarks, docs]`

Steps are named `julia` (the released Julia versions), `nightly`, `cuda` (the CUDA toolkits),
`downstream` (packages depending on CUDA.jl, such as Enzyme.jl), `special` (multi-GPU,
memory types, compute sanitizer, GPU-less environment), `docs` and `benchmarks`. `tests`
covers all but the last two.
