...
* `algo = (CUDACore.math_mode()===CUDACore.FAST_MATH ? CUDNN_SOFTMAX_FAST : CUDNN_SOFTMAX_ACCURATE)`: Options are `CUDNN_SOFTMAX_ACCURATE` which subtracts max from every point to avoid overflow, `CUDNN_SOFTMAX_FAST` which doesn't and `CUDNN_SOFTMAX_LOG` which returns logsoftmax.
...
function cudnnSoftmaxForwardWithDefaults(
    x;
    y = similar(x),
    algo::cudnnSoftmaxAlgorithm_t = (CUDACore.math_mode()===CUDACore.FAST_MATH ? CUDNN_SOFTMAX_FAST : CUDNN_SOFTMAX_ACCURATE),
