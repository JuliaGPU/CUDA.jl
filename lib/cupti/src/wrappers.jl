...
Macro version of `enable!(f, cfg)` that avoids creating a closure,
similar to `@lock` vs `lock(f, l)`.
"""
macro enable!(cfg, expr)
    mod = @__MODULE__
    quote
        _cfg = $(esc(cfg))
        if _cfg isa $mod.ActivityConfig
            @lock $mod.activity_lock begin
                ...
                # Activity support depends on both version and platform: Tegra rejects
                # NVTX marker-data records through CUDA 11. Skip unsupported kinds and
                # keep track of what needs to be disabled, including on setup failure.
...
function enable!(@nospecialize(f::Base.Callable), cfg::ActivityConfig)
    @enable! cfg f()
end
