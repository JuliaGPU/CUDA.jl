...
LinearAlgebra.issymmetric(M::Union{CuSparseMatrixCSC,CuSparseMatrixCSR}) = eltype(M) <: Real ? norm(M - M', Inf) < 1e-8 : false
LinearAlgebra.ishermitian(M::Union{CuSparseMatrixCSC,CuSparseMatrixCSR}) = norm(M - M', Inf) < 1e-8
