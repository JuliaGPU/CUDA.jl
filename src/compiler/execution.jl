...
macro cuda(ex...)
    ...
    @gensym kernel_fields kernel_f kernel_args kernel_tt kernel
    if dynamic
            ...
            quote
                GC.@preserve $(vars...) begin
                    local $kernel_f = if nfields($f) > 0
                        ...
                        local $kernel_fields = Any[$cudaconvert(getfield($f, fld)) for fld in 1:nfields($f)]
                        ccall(:jl_new_structt, Any, (Any, Any),
                              typeof($f).name.wrapper{typeof.($kernel_fields)...}, Tuple($kernel_fields))
                    else
                        $f
                    end
