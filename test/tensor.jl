...
@testset "Elementwise binary" begin
    @testset for (eltyA, eltyB, eltyC) in ((Float32, Float32, Float32),
                                           (Float16, Float16, Float16),
                                           (Float64, Float64, Float64),
                                           (ComplexF16, ComplexF16, ComplexF16),
        ...
        dD = convert(CuArray{eltyC}, copy(dC))
        opAC = CUTENSOR.CUTENSOR_OP_ADD
        dD = CUTENSOR.elementwiseBinary!(one(eltyA), dA, Ainds, Aop, one(eltyB), dC, Cinds, Cop, dD, Cinds, opAC)
        D = collect(dD)
        ...
        ctD = ctA + ctC
        hD = collect(ctD.data)
        @test hD ≈ convert(Array{eltyC}, permutedims(A, [3, 2, 1]) + C)
        ctD = ctA - ctC
        hD = collect(ctD.data)
        @test hD ≈ permutedims(A, [3, 2, 1]) - C
        
        ctD = LinearAlgebra.axpy!(convert(eltyA, 4.), ctA, ctC)
        hD = collect(ctD.data)
        @test hD ≈ convert(eltyA, 4.0)*permutedims(A, [3, 2, 1]) + C
        
        ctD = LinearAlgebra.axpby!(convert(eltyA, 4.), ctA, convert(eltyB, 2.), ctC)
        hD = collect(ctD.data)
        @test hD ≈ convert(eltyA, 4.)*permutedims(A, [3, 2, 1]) + convert(eltyB, 2.)*C
