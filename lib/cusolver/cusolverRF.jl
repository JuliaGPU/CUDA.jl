...
function RfLU(
    ...
    CUDA.@sync cusolverRfAnalyze(rf)
    # LU factorization
    CUDA.@sync cusolverRfRefactor(rf)
...
function rf_refactor!(rflu::RfLU{T}, A::CuSparseMatrixCSR{T}) where T
    CUDA.@sync cusolverRfResetValues(
        rflu.n, rflu.nnzA,
        rflu.drowsA, rflu.dcolsA, A.nzVal, rflu.dP, rflu.dQ,
        rflu.rf
    )
    CUDA.@sync cusolverRfRefactor(rflu.rf)
    return
end
...
function RfBatchLU(
    ...
    # Analyze available parallelism
    CUDA.@sync cusolverRfBatchAnalyze(rf)
    # LU factorization
    CUDA.@sync cusolverRfBatchRefactor(rf)
...
# Update factorization inplace
## Single matrix
function rf_batch_refactor!(rflu::RfBatchLU{T}, A::CuSparseMatrixCSR{T}) where T
...
## Multiple matrices
function rf_batch_refactor!(rflu::RfBatchLU{T}, As::Vector{CuSparseMatrixCSR{T}}) where T
