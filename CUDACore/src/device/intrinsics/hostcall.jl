...
@public HostcallClient, HostcallPort, HostcallHeader, DeviceHostFunction,
...
# Built-in targets occupy the low identifiers, static targets the rest of the low half,
# and runtime HostFunction handles the high half.
const HOSTCALL_BUILTIN_IDS = UInt64(256)
const HOSTCALL_STATIC_ID_MASK = UInt64(0x7fff_ffff_ffff_ffff)
const HOSTCALL_RUNTIME_ID_BIT = UInt64(0x8000_0000_0000_0000)
...
HostcallClient() = HostcallClient(0, reinterpret(LLVMPtr{UInt32,AS.Global}, C_NULL),
...
"""
    DeviceHostFunction{RT,AT}

Device-side handle to a host function registered with `CUDA.HostFunction`; obtained by
passing a `HostFunction` as a kernel argument. Call it like a function (`hf(args...)`) or
use it with [`@hostcall`](@ref).
"""
struct DeviceHostFunction{RT,AT}
    id::UInt64
end
...
function hostcall_target_id_value(@nospecialize(K::Type))
    id = (hash(K) % UInt64) & HOSTCALL_STATIC_ID_MASK
    return id < HOSTCALL_BUILTIN_IDS ? id + HOSTCALL_BUILTIN_IDS : id
end
...
# (`Tuple{typeof(f), RT, AT}`) or `Nothing` for runtime handles. it must not be inlined,
...
Arguments must be isbits; `f` must be recoverable from its type (a named function or an
isbits functor) or a [`DeviceHostFunction`](@ref) handle. See [`@hostcall`](@ref).
"""
@inline function hostcall(f::F, ::Type{RT}, args...) where {F,RT}
    ...
    return hostcall_impl(K, hostcall_target_id(K), RT, payload, Val(false))::RT
end

@inline function hostcall_async(f::F, args...) where {F}
    payload = hostcall_payload(f, map(hostconvert, args))
    K = hostcall_key(f, Nothing, typeof(payload))
    hostcall_impl(K, hostcall_target_id(K), Nothing, payload, Val(true))
    return nothing
end

@doc (@doc hostcall) hostcall_async

# handles: the identifier is a runtime value, and the return type is known from the handle
@inline function hostcall(hf::DeviceHostFunction{RT,AT}, ::Type{RT}, args...) where {RT,AT}
    hostcall_impl(Nothing, hf.id, RT, convert(AT, map(hostconvert, args)), Val(false))::RT
end
@inline hostcall(hf::DeviceHostFunction{RT}, args...) where {RT} = hostcall(hf, RT, args...)
@inline function hostcall_async(hf::DeviceHostFunction{RT,AT}, args...) where {RT,AT}
    hostcall_impl(Nothing, hf.id, Nothing, convert(AT, map(hostconvert, args)), Val(true))
    return nothing
end
@inline (hf::DeviceHostFunction)(args...) = hostcall(hf, args...)
...
own results through a single port; divergent lanes simply form separate calls. Arguments
and results are isbits values shipped in their Julia layout; device memory is passed as
explicit pointers (received as `CuPtr` on the host). The handler runs on a dedicated host
thread with the kernel's context active and a dedicated non-blocking stream; handler
exceptions surface as `HostcallException` at the next `synchronize()`.
