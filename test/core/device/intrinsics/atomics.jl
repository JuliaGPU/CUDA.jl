...
@testset "Float32 atomic denormals" begin
    function kernel(a, b, old, ::Val{local_fastmath}) where {local_fastmath}
        i = threadIdx().x
        if local_fastmath
            @inbounds old[i] = @fastmath CUDA.atomic_add!(pointer(a, i), b[i])
        else
            @inbounds old[i] = CUDA.atomic_add!(pointer(a, i), b[i])
        end
        return
    end

    tiny = nextfloat(0.0f0)
    small = floatmin(Float32)
    initial = Float32[0, -0.0, tiny, -tiny, small, -small, 1]
    values = Float32[tiny, -tiny, tiny, -tiny, -nextfloat(small), nextfloat(small), 2]
    flush(x) = issubnormal(x) ? copysign(0.0f0, x) : x

    for fastmath in (false, true), local_fastmath in (false, true)
        a, b = CuArray(initial), CuArray(values)
        old = similar(a)
        @cuda threads=length(a) fastmath=fastmath kernel(a, b, old, Val(local_fastmath))
        @test reinterpret(UInt32, Array(old)) == reinterpret(UInt32, initial)
        expected = fastmath ? map((x, y) -> flush(flush(x) + flush(y)), initial, values) :
                              initial .+ values
        @test isequal(Array(a), expected)

        tt = Tuple{CuDeviceVector{Float32,1}, CuDeviceVector{Float32,1},
                   CuDeviceVector{Float32,1}, Val{local_fastmath}}
        ptx = sprint(io -> CUDA.code_ptx(io, kernel, tt; fastmath))
        if fastmath
            @test occursin(r"atom\.[^\n]*add\.f32", ptx)
            @test !occursin("cas.b32", ptx)
        else
            # LLVM 23 expands global IEEE fadd to preserve subnormals.
            @test occursin("cas.b32", ptx)
        end
    end
end
