...
struct coalesced_group <: thread_group
    ...
    coalesced_group(mask::UInt32) = new(mask, CUDACore.popc(mask), 0, 1)
end
...
thread_rank(cg::coalesced_group) = CUDACore.popc(cg.mask & CUDACore.lanemask(<)) + 1i32
...
function shfl(cg::coalesced_group, elem, src_rank)
    lane = if src_rank == 0
        CUDACore.ffs(cg.mask)
    elseif num_threads(cg) == 32
        src_rank
    else
        CUDACore.fns(cg.mask, 0, src_rank) + 1i32
    end
...
function shfl_down(cg::coalesced_group, elem, delta)
    ...
    lane = CUDACore.fns(cg.mask, laneid() - 1i32, delta + 1i32) + 1i32
