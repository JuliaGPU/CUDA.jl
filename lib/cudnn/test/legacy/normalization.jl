...
s = CuArray(randn(Float32, 1,1,3,1))
normtest(x)
...
normtest(x; scale = fill!(similar(s), 2))
normtest(x; bias  = fill!(similar(s), 2))
normtest(x; xmean  = fill!(similar(s), 2))
normtest(x; xvar = fill!(similar(s), 2))
