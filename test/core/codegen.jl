...
@testset "relocation patching" begin
    ...
    @test Array(out)[] == 1
    # no per-kernel root bookkeeping: resolving the manifest permanently rooted `Float64`
    # in the process (`jl_as_global_root`), so there is nothing kernel-local to assert
end
