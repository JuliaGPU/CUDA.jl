...
# `nothing` when dispatch fails or needs argument values; the service path then dispatches
# in the latest world and reports an ordinary MethodError if no method applies. Resolution
# runs in the latest world explicitly: the server task remains in its adoption world.
...
function resolve_hostcall_method(target::HostcallTarget, world::UInt)
    try
        ...
        # A payload type need not identify a dispatch signature: DataType, for example,
        # can hold Int or Float32, which select different Type{T} methods. In that case
        # dispatch on the received values instead of invoking a less-specific method.
        Base.isdispatchtuple(sig) || return nothing
        precompile(sig)
        ft = sig.parameters[1]
...
function service_target!(p::HostPort, target::HostcallTarget, hdr::HostcallHeader)
    ...
    # Include deferred formatting in the latest world: show methods can be defined after
    # the server starts, just like ordinary handlers. Use one snapshot for every lane.
    world, mi = method === nothing ? (Base.get_world_counter(), nothing) : method
    Base.invoke_in_world(world, service_lanes!, p, target, hdr, mi)
    return
end
