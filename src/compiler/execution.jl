        ...
        push!(code.args,
            quote
                GC.@preserve $(vars...) begin
                    local $kernel_f = if nfields($f) > 0
                        # rewrite captured variables (static parameters are placed first)
                        nsparams = length(typeof($f).parameters) - nfields($f)
                        sparams = ntuple(i->typeof($f).parameters[i], nsparams)
                        fields = ntuple(i->$cudaconvert(getfield($f, i)), nfields($f))
                        ftyp = typeof($f).name.wrapper{sparams...,
                                                       map(Core.Typeof, fields)...}
