    ...
    if dynamic
        ...
        push!(code.args,
            quote
                ...
                dynamic_launch(kernel, $(var_exprs...); $(map(esc, call_kwargs)...))
             end)
    else
...
@inline function dynamic_launch(f::Ptr{Cvoid}, values::Vararg{Any,N};
                                blocks::CuDim=1, threads::CuDim=1, shmem::Int=0,
