...
# the iterative-refinement (IRS) API is only available since cuSOLVER 11
if cuSOLVER.version() >= v"11"
    ...
    @testset "unsupported iterative refinement" begin
        A = CuArray(Float32[2 0; 0 3])
        B = CuArray(Float32[1, 2])
        @test_throws "cuSOLVER 11" cuSOLVER.gesv!(similar(B), copy(A), B)
        @test_throws "cuSOLVER 11" cuSOLVER.gels!(similar(B), copy(A), B)
    end
end
