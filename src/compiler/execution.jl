...
macro cuda(ex...)
    ...
    @gensym kernel_fields kernel_type kernel_f kernel_args kernel_tt kernel
    if dynamic
            ...
            quote
                GC.@preserve $(vars...) begin
                    local $kernel_f = if nfields($f) > 0
                        # rewrite captured variables
                        # TODO: this assumes the function's typevars match its fields. this
                        #       is true for the closures generated by Julia's front-end,
                        #       but might not be for arbitrary callable objects. however,
                        #       we currently don't support those (assuming <: Function).
                        local $kernel_fields = Tuple($cudaconvert(getfield($f, fld)) for fld in 1:nfields($f))
                        local $kernel_type = typeof($f).name.wrapper{typeof.($kernel_fields)...}
                        $(Expr(:splatnew, kernel_type, kernel_fields))
                    else
                        $f
                    end
