            ...
            quote
                GC.@preserve $(vars...) begin
                    local $kernel_f = if nfields($f) > 0
                        ...
                        # TODO: this assumes typevars match parameters (barring static
                        #       parameters). that's true for closures generated by Julia,
                        #       but not for arbitrary callable objects.
                        # TODO: convert the static parameters, because code will assume
                        #       types match, but cudaconvert only works on values.
