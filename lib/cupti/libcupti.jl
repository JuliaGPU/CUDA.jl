...
struct CUpti_SubscriberParams
    structSize::Csize_t
    subscriberName::Cstring
    oldSubscriberName::Cstring
    oldSubscriberSize::Csize_t
    allowMultipleSubscribers::UInt8
    padding::NTuple{7,UInt8}
end

struct CUpti_ActivityFieldSelection
    structSize::Csize_t
    numFields::Csize_t
    pFieldIds::Ptr{Cint}
end
...
struct CUpti_ActivityConfig
    structSize::Csize_t
    fieldSelection::CUpti_ActivityFieldSelection
end
...
function Base.propertynames(x::var"##Ctag#382", private::Bool=false)
...
function Base.getproperty(x::Ptr{CUpti_ResourceData}, f::Symbol)
    ...
    f === :resourceHandle && return Ptr{var"##Ctag#382"}(x + 8)
...
function Base.propertynames(x::CUpti_ResourceData, private::Bool=false)
    return (:context, :resourceHandle, :resourceDescriptor, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::CUpti_StateData, private::Bool=false)
    return (:notification, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
@checked function cuptiSubscribe_v2(subscriber, callback, userdata, pParams)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiSubscribe_v2(subscriber::Ptr{CUpti_SubscriberHandle},
                                             callback::CUpti_CallbackFunc,
                                             userdata::Ptr{Cvoid},
                                             pParams::Ptr{CUpti_SubscriberParams})::CUptiResult
end
...
@checked function cuptiGetEnabledCallbacks(subscriber, domain, buffer, bufferSize,
                                           enabledCallbacksCount)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiGetEnabledCallbacks(subscriber::CUpti_SubscriberHandle,
                                                    domain::CUpti_CallbackDomain,
                                                    buffer::Ptr{CUpti_CallbackId},
                                                    bufferSize::Ptr{UInt32},
                                                    enabledCallbacksCount::Ptr{UInt32})::CUptiResult
end
...
function Base.getproperty(x::Ptr{CUpti_MetricValue}, f::Symbol)
    ...
    f === :metricValueNvtxExtendedPayload && return Ptr{UInt64}(x + 0)
    return getfield(x, f)
end
...
function Base.propertynames(x::CUpti_MetricValue, private::Bool=false)
    return (:metricValueDouble, :metricValueUint64, :metricValueInt64, :metricValuePercent,
            :metricValueThroughput, :metricValueUtilizationLevel,
            :metricValueNvtxExtendedPayload, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.getproperty(x::Ptr{CUpti_ActivityObjectKindId}, f::Symbol)
    f === :pt && return Ptr{var"##Ctag#398"}(x + 0)
    f === :dcs && return Ptr{var"##Ctag#399"}(x + 0)
    return getfield(x, f)
end
...
function Base.propertynames(x::CUpti_ActivityObjectKindId, private::Bool=false)
    return (:pt, :dcs, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
@cenum CUpti_NvtxExtPayloadType::UInt32 begin
...
function Base.propertynames(x::CUpti_ActivityUnifiedMemoryCounterConfig,
                            private::Bool=false)
    return (:scope, :kind, :deviceId, :enable, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::CUpti_ActivityAutoBoostState, private::Bool=false)
    return (:enabled, :pid, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::CUpti_ActivityPCSamplingConfig, private::Bool=false)
    return (:size, :samplingPeriod, :samplingPeriod2, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
struct CUpti_ActivityFieldLayoutEntry
    structSize::Csize_t
    fieldId::Cint
    offset::Csize_t
    size::Csize_t
...
struct CUpti_ActivityRecordLayout
    structSize::Csize_t
    pEntries::Ptr{CUpti_ActivityFieldLayoutEntry}
    numFields::Csize_t
    recordSize::Csize_t
end
...
function Base.propertynames(x::CUpti_Activity, private::Bool=false)
    return (:kind, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.getproperty(x::Ptr{CUpti_ActivityMemcpy6}, f::Symbol)
    ...
    f === :isDeviceLaunched && return Ptr{UInt8}(x + 84)
    f === :pad2 && return Ptr{NTuple{3,UInt8}}(x + 85)
...
function Base.propertynames(x::CUpti_ActivityMemcpy6, private::Bool=false)
    return (:kind, :copyKind, :srcKind, :dstKind, :flags, :bytes, :start, :_end, :deviceId,
            :contextId, :streamId, :correlationId, :runtimeCorrelationId, :pad, :reserved0,
            :graphNodeId, :graphId, :channelID, :channelType, :isDeviceLaunched, :pad2,
            :copyCount, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
@cenum CUpti_ActivityMemcpyFieldIds::UInt32 begin
    MEMCPY_FIELD_KIND = 0
...
function Base.propertynames(x::CUpti_ActivityMemcpyPtoP4, private::Bool=false)
    return (:kind, :copyKind, :srcKind, :dstKind, :flags, :bytes, :start, :_end, :deviceId,
            :contextId, :streamId, :srcDeviceId, :srcContextId, :dstDeviceId, :dstContextId,
            :correlationId, :reserved0, :graphNodeId, :graphId, :channelID, :channelType,
            if private
                fieldnames(typeof(x))
            else
                ()
...
@cenum CUpti_ActivityMemcpy2FieldIds::UInt32 begin
...
function Base.propertynames(x::CUpti_ActivityMemset4, private::Bool=false)
    return (:kind, :value, :bytes, :start, :_end, :deviceId, :contextId, :streamId,
            :correlationId, :flags, :memoryKind, :pad, :reserved0, :graphNodeId, :graphId,
            :channelID, :channelType, :isDeviceLaunched, :pad2,
            if private
                fieldnames(typeof(x))
            else
                ()
...
@cenum CUpti_ActivityMemsetFieldIds::UInt32 begin
    MEMSET_FIELD_KIND = 0
...
function Base.propertynames(x::CUpti_ActivityMemory, private::Bool=false)
    return (:kind, :memoryKind, :address, :bytes, :start, :_end, :allocPC, :freePC,
            :processId, :deviceId, :contextId, :pad, :name, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::var"##Ctag#311", private::Bool=false)
...
function Base.getproperty(x::Ptr{var"##Ctag#310"}, f::Symbol)
    ...
    f === :pool && return Ptr{var"##Ctag#311"}(x + 24)
...
function Base.propertynames(x::CUpti_ActivityMemory4, private::Bool=false)
    return (:kind, :memoryOperationType, :memoryKind, :correlationId, :address, :bytes,
            :timestamp, :PC, :processId, :deviceId, :contextId, :streamId, :name, :isAsync,
            :pad1, :memoryPoolConfig, :source, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
@cenum CUpti_ActivityMemoryFieldIds::UInt32 begin
    MEMORY_FIELD_KIND = 0
...
function Base.getproperty(x::Ptr{CUpti_ActivityMemoryPool3}, f::Symbol)
    f === :kind && return Ptr{CUpti_ActivityKind}(x + 0)
    ...
    f === :isManagedPool && return Ptr{UInt8}(x + 72)
    f === :pad2 && return Ptr{NTuple{7,UInt8}}(x + 73)
    return getfield(x, f)
end

function Base.getproperty(x::CUpti_ActivityMemoryPool3, f::Symbol)
    r = Ref{CUpti_ActivityMemoryPool3}(x)
    ptr = Base.unsafe_convert(Ptr{CUpti_ActivityMemoryPool3}, r)
    fptr = getproperty(ptr, f)
    GC.@preserve r unsafe_load(fptr)
end

function Base.setproperty!(x::Ptr{CUpti_ActivityMemoryPool3}, f::Symbol, v)
    return unsafe_store!(getproperty(x, f), v)
end

function Base.propertynames(x::CUpti_ActivityMemoryPool3, private::Bool=false)
    return (:kind, :memoryPoolOperationType, :memoryPoolType, :correlationId, :processId,
            :deviceId, :minBytesToKeep, :address, :size, :releaseThreshold, :timestamp,
            :utilizedSize, :isManagedPool, :pad2, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
@cenum CUpti_ActivityMemoryPoolFieldIds::UInt32 begin
...
function Base.getproperty(x::Ptr{var"##Ctag#400"}, f::Symbol)
    ...
    f === :config && return Ptr{var"##Ctag#401"}(x + 0)
    return getfield(x, f)
end
...
function Base.getproperty(x::Ptr{CUpti_ActivityKernel11}, f::Symbol)
    f === :kind && return Ptr{CUpti_ActivityKind}(x + 0)
    f === :cacheConfig && return Ptr{var"##Ctag#400"}(x + 4)
    ...
    f === :isDeviceLaunched && return Ptr{UInt8}(x + 208)
    f === :padding3 && return Ptr{NTuple{7,UInt8}}(x + 209)
    f === :priority && return Ptr{Int32}(x + 216)
    f === :padding4 && return Ptr{UInt32}(x + 220)
    return getfield(x, f)
end

function Base.getproperty(x::CUpti_ActivityKernel11, f::Symbol)
    r = Ref{CUpti_ActivityKernel11}(x)
    ptr = Base.unsafe_convert(Ptr{CUpti_ActivityKernel11}, r)
    fptr = getproperty(ptr, f)
    GC.@preserve r unsafe_load(fptr)
end

function Base.setproperty!(x::Ptr{CUpti_ActivityKernel11}, f::Symbol, v)
    return unsafe_store!(getproperty(x, f), v)
end

function Base.propertynames(x::CUpti_ActivityKernel11, private::Bool=false)
    return (:kind, :cacheConfig, :sharedMemoryConfig, :registersPerThread,
            :partitionedGlobalCacheRequested, :partitionedGlobalCacheExecuted, :start,
            :_end, :completed, :deviceId, :contextId, :streamId, :gridX, :gridY, :gridZ,
            :blockX, :blockY, :blockZ, :staticSharedMemory, :dynamicSharedMemory,
            :localMemoryPerThread, :localMemoryTotal, :correlationId, :gridId, :name,
            :reserved0, :queued, :submitted, :launchType, :isSharedMemoryCarveoutRequested,
            :sharedMemoryCarveoutRequested, :padding, :sharedMemoryExecuted, :graphNodeId,
            :shmemLimitConfig, :graphId, :pAccessPolicyWindow, :channelID, :channelType,
            :clusterX, :clusterY, :clusterZ, :clusterSchedulingPolicy, :localMemoryTotal_v2,
            :maxPotentialClusterSize, :maxActiveClusters, :isDeviceLaunched, :padding3,
            :priority, :padding4, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
@cenum CUpti_ActivityKernelFieldIds::UInt32 begin
    KERNEL_FIELD_KIND = 0
...
function Base.getproperty(x::Ptr{var"##Ctag#308"}, f::Symbol)
    f === :both && return Ptr{UInt8}(x + 0)
    f === :config && return Ptr{var"##Ctag#309"}(x + 0)
    return getfield(x, f)
end
...
function Base.getproperty(x::Ptr{CUpti_ActivityCdpKernel}, f::Symbol)
    f === :kind && return Ptr{CUpti_ActivityKind}(x + 0)
    f === :cacheConfig && return Ptr{var"##Ctag#308"}(x + 4)
    f === :sharedMemoryConfig && return Ptr{UInt8}(x + 5)
    f === :registersPerThread && return Ptr{UInt16}(x + 6)
...
function Base.propertynames(x::CUpti_ActivityCdpKernel, private::Bool=false)
    return (:kind, :cacheConfig, :sharedMemoryConfig, :registersPerThread, :start, :_end,
            :deviceId, :contextId, :streamId, :gridX, :gridY, :gridZ, :blockX, :blockY,
            :blockZ, :staticSharedMemory, :dynamicSharedMemory, :localMemoryPerThread,
            :localMemoryTotal, :correlationId, :gridId, :parentGridId, :queued, :submitted,
            :completed, :parentBlockX, :parentBlockY, :parentBlockZ, :pad, :name,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivityPreemption, private::Bool=false)
    return (:kind, :preemptionKind, :timestamp, :gridId, :blockX, :blockY, :blockZ, :pad,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivityAPI, private::Bool=false)
    return (:kind, :cbid, :start, :_end, :processId, :threadId, :correlationId,
            :returnValue, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
@cenum CUpti_ActivityApiFieldIds::UInt32 begin
    API_FIELD_KIND = 0
    API_FIELD_CBID = 1
    API_FIELD_START = 2
    API_FIELD_END = 3
...
function Base.propertynames(x::CUpti_ActivityEvent, private::Bool=false)
    return (:kind, :id, :value, :domain, :correlationId, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::CUpti_ActivityEventInstance, private::Bool=false)
    return (:kind, :id, :domain, :instance, :value, :correlationId, :pad,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivityMetric, private::Bool=false)
    return (:kind, :id, :value, :correlationId, :flags, :pad, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::CUpti_ActivityMetricInstance, private::Bool=false)
    return (:kind, :id, :value, :instance, :correlationId, :flags, :pad,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivitySourceLocator, private::Bool=false)
    return (:kind, :id, :lineNumber, :pad, :fileName, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::CUpti_ActivityGlobalAccess3, private::Bool=false)
    return (:kind, :flags, :sourceLocatorId, :correlationId, :functionId, :executed,
            :pcOffset, :threadsExecuted, :l2_transactions, :theoreticalL2Transactions,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivityBranch2, private::Bool=false)
    return (:kind, :sourceLocatorId, :correlationId, :functionId, :pcOffset, :diverged,
            :threadsExecuted, :executed, :pad, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.getproperty(x::Ptr{CUpti_ActivityDevice6}, f::Symbol)
    f === :kind && return Ptr{CUpti_ActivityKind}(x + 0)
    f === :flags && return Ptr{CUpti_ActivityFlag}(x + 4)
    ...
    f === :numTpcs && return Ptr{UInt32}(x + 184)
    f === :reserved0 && return Ptr{UInt32}(x + 188)
    return getfield(x, f)
end

function Base.getproperty(x::CUpti_ActivityDevice6, f::Symbol)
    r = Ref{CUpti_ActivityDevice6}(x)
    ptr = Base.unsafe_convert(Ptr{CUpti_ActivityDevice6}, r)
    fptr = getproperty(ptr, f)
    GC.@preserve r unsafe_load(fptr)
end

function Base.setproperty!(x::Ptr{CUpti_ActivityDevice6}, f::Symbol, v)
    return unsafe_store!(getproperty(x, f), v)
end

function Base.propertynames(x::CUpti_ActivityDevice6, private::Bool=false)
    return (:kind, :flags, :globalMemoryBandwidth, :globalMemorySize, :constantMemorySize,
            :l2CacheSize, :numThreadsPerWarp, :coreClockRate, :numMemcpyEngines,
            :numMultiprocessors, :maxIPC, :maxWarpsPerMultiprocessor,
            :maxBlocksPerMultiprocessor, :maxSharedMemoryPerMultiprocessor,
            :maxRegistersPerMultiprocessor, :maxRegistersPerBlock, :maxSharedMemoryPerBlock,
            :maxThreadsPerBlock, :maxBlockDimX, :maxBlockDimY, :maxBlockDimZ, :maxGridDimX,
            :maxGridDimY, :maxGridDimZ, :computeCapabilityMajor, :computeCapabilityMinor,
            :id, :eccEnabled, :uuid, :name, :isCudaVisible, :isMigEnabled, :reserved,
            :gpuInstanceId, :computeInstanceId, :migUuid, :isNumaNode, :numaId, :numTpcs,
...
@cenum CUpti_ActivityDeviceFieldIds::UInt32 begin
    DEVICE_FIELD_KIND = 0
...
function Base.propertynames(x::var"##Ctag#368", private::Bool=false)
...
function Base.propertynames(x::var"##Ctag#369", private::Bool=false)
...
function Base.getproperty(x::Ptr{CUpti_ActivityDeviceAttribute}, f::Symbol)
    ...
    f === :flags && return Ptr{CUpti_ActivityFlag}(x + 4)
    f === :deviceId && return Ptr{UInt32}(x + 8)
    f === :attribute && return Ptr{var"##Ctag#368"}(x + 12)
    f === :value && return Ptr{var"##Ctag#369"}(x + 16)
    return getfield(x, f)
end
...
function Base.propertynames(x::CUpti_ActivityDeviceAttribute, private::Bool=false)
    return (:kind, :flags, :deviceId, :attribute, :value, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.getproperty(x::Ptr{CUpti_ActivityContext4}, f::Symbol)
    f === :kind && return Ptr{CUpti_ActivityKind}(x + 0)
    ...
    f === :processId && return Ptr{UInt64}(x + 32)
    return getfield(x, f)
end

function Base.getproperty(x::CUpti_ActivityContext4, f::Symbol)
    r = Ref{CUpti_ActivityContext4}(x)
    ptr = Base.unsafe_convert(Ptr{CUpti_ActivityContext4}, r)
    fptr = getproperty(ptr, f)
    GC.@preserve r unsafe_load(fptr)
end

function Base.setproperty!(x::Ptr{CUpti_ActivityContext4}, f::Symbol, v)
    return unsafe_store!(getproperty(x, f), v)
end

function Base.propertynames(x::CUpti_ActivityContext4, private::Bool=false)
    return (:kind, :contextId, :deviceId, :computeApiKind, :nullStreamId, :parentContextId,
            :isGreenContext, :padding, :numMultiprocessors, :cigMode, :padding2, :processId,
            if private
                fieldnames(typeof(x))
            else
                ()
...
@cenum CUpti_ActivityContextFieldIds::UInt32 begin
...
function Base.propertynames(x::CUpti_ActivityName, private::Bool=false)
    return (:kind, :objectKind, :objectId, :pad, :name, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
@cenum CUpti_ActivityNameFieldIds::UInt32 begin
    NAME_FIELD_KIND = 0
...
function Base.propertynames(x::CUpti_ActivityMarker2, private::Bool=false)
    return (:kind, :flags, :timestamp, :id, :objectKind, :objectId, :pad, :name, :domain,
            if private
                fieldnames(typeof(x))
            else
                ()
...
@cenum CUpti_ActivityMarkerFieldIds::UInt32 begin
    MARKER_FIELD_KIND = 0
...
function Base.getproperty(x::Ptr{CUpti_ActivityMarkerData2}, f::Symbol)
    f === :kind && return Ptr{CUpti_ActivityKind}(x + 0)
    f === :flags && return Ptr{CUpti_ActivityFlag}(x + 4)
    ...
    f === :cuptiDomainId && return Ptr{UInt32}(x + 32)
    f === :padding && return Ptr{UInt32}(x + 36)
    return getfield(x, f)
end

function Base.getproperty(x::CUpti_ActivityMarkerData2, f::Symbol)
    r = Ref{CUpti_ActivityMarkerData2}(x)
    ptr = Base.unsafe_convert(Ptr{CUpti_ActivityMarkerData2}, r)
    fptr = getproperty(ptr, f)
    GC.@preserve r unsafe_load(fptr)
end

function Base.setproperty!(x::Ptr{CUpti_ActivityMarkerData2}, f::Symbol, v)
    return unsafe_store!(getproperty(x, f), v)
end

function Base.propertynames(x::CUpti_ActivityMarkerData2, private::Bool=false)
    return (:kind, :flags, :id, :payloadKind, :payload, :color, :category, :cuptiDomainId,
            :padding, if private
                fieldnames(typeof(x))
            else
                ()
...
@cenum CUpti_ActivityMarkerDataFieldIds::UInt32 begin
...
function Base.propertynames(x::CUpti_ActivityOverhead3, private::Bool=false)
    return (:kind, :overheadKind, :objectKind, :objectId, :start, :_end, :correlationId,
            :reserved0, :overheadData, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
@cenum CUpti_ActivityOverheadFieldIds::UInt32 begin
...
function Base.getproperty(x::Ptr{var"##Ctag#357"}, f::Symbol)
    f === :speed && return Ptr{var"##Ctag#358"}(x + 0)
    f === :temperature && return Ptr{var"##Ctag#359"}(x + 0)
    f === :power && return Ptr{var"##Ctag#360"}(x + 0)
    f === :cooling && return Ptr{var"##Ctag#361"}(x + 0)
    return getfield(x, f)
end
...
function Base.getproperty(x::Ptr{CUpti_ActivityEnvironment}, f::Symbol)
    ...
    f === :data && return Ptr{var"##Ctag#357"}(x + 20)
    return getfield(x, f)
end
...
function Base.propertynames(x::CUpti_ActivityEnvironment, private::Bool=false)
    return (:kind, :deviceId, :timestamp, :environmentKind, :data,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivityInstructionExecution, private::Bool=false)
    return (:kind, :flags, :sourceLocatorId, :correlationId, :functionId, :pcOffset,
            :threadsExecuted, :notPredOffThreadsExecuted, :executed, :pad,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivityPCSampling3, private::Bool=false)
    return (:kind, :flags, :sourceLocatorId, :correlationId, :functionId, :latencySamples,
            :samples, :stallReason, :pcOffset, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::CUpti_ActivityPCSamplingRecordInfo, private::Bool=false)
    return (:kind, :correlationId, :totalSamples, :droppedSamples, :samplingPeriodInCycles,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivityUnifiedMemoryCounter3, private::Bool=false)
    return (:kind, :counterKind, :value, :start, :_end, :address, :srcId, :dstId, :streamId,
            :processId, :flags, :pad, :processors, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
@cenum CUpti_ActivityUvmCounterFieldIds::UInt32 begin
...
function Base.propertynames(x::CUpti_ActivityFunction, private::Bool=false)
    return (:kind, :id, :contextId, :moduleId, :functionIndex, :pad, :name,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivityModule, private::Bool=false)
    return (:kind, :contextId, :id, :cubinSize, :cubin, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::CUpti_ActivitySharedAccess, private::Bool=false)
    return (:kind, :flags, :sourceLocatorId, :correlationId, :functionId, :pcOffset,
            :threadsExecuted, :sharedTransactions, :theoreticalSharedTransactions,
...
function Base.propertynames(x::CUpti_ActivityCudaEvent2, private::Bool=false)
    return (:kind, :correlationId, :contextId, :streamId, :eventId, :pad, :deviceId, :pad2,
            :reserved0, :deviceTimestamp, :cudaEventSyncId, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
@cenum CUpti_ActivityCudaEventFieldIds::UInt32 begin
...
function Base.propertynames(x::CUpti_ActivityStream, private::Bool=false)
    return (:kind, :contextId, :streamId, :priority, :flag, :correlationId,
            if private
                fieldnames(typeof(x))
            else
                ()
...
@cenum CUpti_ActivityStreamFieldIds::UInt32 begin
    STREAM_FIELD_KIND = 0
...
function Base.propertynames(x::CUpti_ActivitySynchronization2, private::Bool=false)
    return (:kind, :type, :start, :_end, :correlationId, :contextId, :streamId,
            :cudaEventId, :cudaEventSyncId, :returnValue, :pad,
            if private
                fieldnames(typeof(x))
            else
                ()
...
@cenum CUpti_ActivitySynchronizationFieldIds::UInt32 begin
...
function Base.propertynames(x::CUpti_ActivityInstructionCorrelation, private::Bool=false)
    return (:kind, :flags, :sourceLocatorId, :functionId, :pcOffset, :pad,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivityOpenAcc, private::Bool=false)
    return (:kind, :eventKind, :parentConstruct, :version, :implicit, :deviceType,
            :deviceNumber, :threadId, :async, :asyncMap, :lineNo, :endLineNo, :funcLineNo,
            :funcEndLineNo, :start, :_end, :cuDeviceId, :cuContextId, :cuStreamId,
            :cuProcessId, :cuThreadId, :externalId, :srcFile, :funcName,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivityOpenAccData, private::Bool=false)
    return (:kind, :eventKind, :parentConstruct, :version, :implicit, :deviceType,
            :deviceNumber, :threadId, :async, :asyncMap, :lineNo, :endLineNo, :funcLineNo,
            :funcEndLineNo, :start, :_end, :cuDeviceId, :cuContextId, :cuStreamId,
            :cuProcessId, :cuThreadId, :externalId, :srcFile, :funcName, :bytes, :hostPtr,
            :devicePtr, :varName, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::CUpti_ActivityOpenAccLaunch, private::Bool=false)
    return (:kind, :eventKind, :parentConstruct, :version, :implicit, :deviceType,
            :deviceNumber, :threadId, :async, :asyncMap, :lineNo, :endLineNo, :funcLineNo,
            :funcEndLineNo, :start, :_end, :cuDeviceId, :cuContextId, :cuStreamId,
            :cuProcessId, :cuThreadId, :externalId, :srcFile, :funcName, :numGangs,
            :numWorkers, :vectorLength, :kernelName, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::CUpti_ActivityOpenAccOther, private::Bool=false)
    return (:kind, :eventKind, :parentConstruct, :version, :implicit, :deviceType,
            :deviceNumber, :threadId, :async, :asyncMap, :lineNo, :endLineNo, :funcLineNo,
            :funcEndLineNo, :start, :_end, :cuDeviceId, :cuContextId, :cuStreamId,
            :cuProcessId, :cuThreadId, :externalId, :srcFile, :funcName,
...
function Base.propertynames(x::CUpti_ActivityOpenMp, private::Bool=false)
    return (:kind, :eventKind, :version, :threadId, :start, :_end, :cuProcessId,
            :cuThreadId, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::CUpti_ActivityExternalCorrelation, private::Bool=false)
    return (:kind, :externalKind, :externalId, :correlationId, :reserved,
            if private
                fieldnames(typeof(x))
            else
                ()
...
@cenum CUpti_ActivityExternalCorrelationFieldIds::UInt32 begin
...
function Base.getproperty(x::Ptr{var"##Ctag#402"}, f::Symbol)
    ...
    f === :npu && return Ptr{var"##Ctag#403"}(x + 0)
    return getfield(x, f)
end
...
function Base.propertynames(x::var"##Ctag#402", private::Bool=false)
...
function Base.getproperty(x::Ptr{var"##Ctag#404"}, f::Symbol)
    f === :uuidDev && return Ptr{CUuuid}(x + 0)
    f === :npu && return Ptr{var"##Ctag#405"}(x + 0)
    return getfield(x, f)
end
...
function Base.getproperty(x::Ptr{CUpti_ActivityNvLink5}, f::Symbol)
    f === :kind && return Ptr{CUpti_ActivityKind}(x + 0)
    f === :nvlinkVersion && return Ptr{UInt32}(x + 4)
    f === :typeDev0 && return Ptr{CUpti_DevType}(x + 8)
    f === :typeDev1 && return Ptr{CUpti_DevType}(x + 12)
    f === :idDev0 && return Ptr{var"##Ctag#402"}(x + 16)
    f === :idDev1 && return Ptr{var"##Ctag#404"}(x + 32)
    f === :flag && return Ptr{UInt32}(x + 48)
    f === :physicalNvLinkCount && return Ptr{UInt32}(x + 52)
    f === :portDev0 && return Ptr{Ptr{UInt32}}(x + 56)
    f === :portDev1 && return Ptr{Ptr{UInt32}}(x + 64)
    f === :bandwidth && return Ptr{UInt64}(x + 72)
    f === :nvswitchConnected && return Ptr{UInt8}(x + 80)
    f === :pad && return Ptr{NTuple{7,UInt8}}(x + 81)
    return getfield(x, f)
end

function Base.getproperty(x::CUpti_ActivityNvLink5, f::Symbol)
    r = Ref{CUpti_ActivityNvLink5}(x)
    ptr = Base.unsafe_convert(Ptr{CUpti_ActivityNvLink5}, r)
    fptr = getproperty(ptr, f)
    GC.@preserve r unsafe_load(fptr)
end

function Base.setproperty!(x::Ptr{CUpti_ActivityNvLink5}, f::Symbol, v)
    return unsafe_store!(getproperty(x, f), v)
end

function Base.propertynames(x::CUpti_ActivityNvLink5, private::Bool=false)
    return (:kind, :nvlinkVersion, :typeDev0, :typeDev1, :idDev0, :idDev1, :flag,
            :physicalNvLinkCount, :portDev0, :portDev1, :bandwidth, :nvswitchConnected,
            :pad, if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.getproperty(x::Ptr{var"##Ctag#363"}, f::Symbol)
    f === :gpuAttr && return Ptr{var"##Ctag#364"}(x + 0)
    f === :bridgeAttr && return Ptr{var"##Ctag#365"}(x + 0)
    return getfield(x, f)
end
...
function Base.getproperty(x::Ptr{CUpti_ActivityPcie}, f::Symbol)
    ...
    f === :id && return Ptr{var"##Ctag#362"}(x + 8)
    ...
    f === :attr && return Ptr{var"##Ctag#363"}(x + 24)
    return getfield(x, f)
end
...
function Base.propertynames(x::CUpti_ActivityPcie, private::Bool=false)
    return (:kind, :type, :id, :domain, :pcieGeneration, :linkRate, :linkWidth,
            :upstreamBus, :attr, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::CUpti_ActivityConfidentialComputeRotation,
                            private::Bool=false)
    return (:kind, :eventType, :deviceId, :contextId, :channelId, :channelType, :timestamp,
            if private
                fieldnames(typeof(x))
            else
                ()
...
@cenum CUpti_ActivityConfidentialComputeRotationFieldIds::UInt32 begin
...
function Base.propertynames(x::CUpti_ActivityInstantaneousEvent, private::Bool=false)
    return (:kind, :id, :value, :timestamp, :deviceId, :reserved,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivityInstantaneousEventInstance,
                            private::Bool=false)
    return (:kind, :id, :value, :timestamp, :deviceId, :instance, :pad,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivityInstantaneousMetric, private::Bool=false)
    return (:kind, :id, :value, :timestamp, :deviceId, :flags, :pad,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivityInstantaneousMetricInstance,
                            private::Bool=false)
    return (:kind, :id, :value, :timestamp, :deviceId, :flags, :instance, :pad,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivityJit2, private::Bool=false)
    return (:kind, :jitEntryType, :jitOperationType, :deviceId, :start, :_end,
            :correlationId, :padding, :jitOperationCorrelationId, :cacheSize, :cachePath,
            :processId, :threadId, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
@cenum CUpti_ActivityJitFieldIds::UInt32 begin
    JIT_FIELD_KIND = 0
...
@cenum CUpti_ActivityGraphTraceFieldIds::UInt32 begin
...
@cenum CUpti_ActivityDeviceGraphTraceFieldIds::UInt32 begin
...
@cenum CUpti_ActivityMemDecompressFieldIds::UInt32 begin
...
@cenum CUpti_ActivityGraphHostNodeFieldIds::UInt32 begin
...
@cenum CUpti_ComputeEngineCtxSwitchOperationType::UInt32 begin
...
struct CUpti_ActivityComputeEngineCtxSwitch
    ...
    contextId::UInt32
    timestamp::UInt64
    operationType::CUpti_ComputeEngineCtxSwitchOperationType
    padding::UInt32
end

@cenum CUpti_ActivityComputeEngineCtxSwitchFieldIds::UInt32 begin
...
@cenum CUpti_ActivityHostLaunchFieldIds::UInt32 begin
...
function Base.getproperty(x::Ptr{CUpti_ActivityGreenContext}, f::Symbol)
    f === :kind && return Ptr{CUpti_ActivityKind}(x + 0)
    f === :contextId && return Ptr{UInt32}(x + 4)
    f === :parentContextId && return Ptr{UInt32}(x + 8)
    f === :deviceId && return Ptr{UInt32}(x + 12)
    f === :numTpcs && return Ptr{UInt32}(x + 16)
    f === :numMultiprocessors && return Ptr{UInt16}(x + 20)
    f === :logicalTpcMaskSize && return Ptr{UInt8}(x + 22)
    f === :padding && return Ptr{UInt8}(x + 23)
    f === :logicalTpcMask && return Ptr{NTuple{32,UInt32}}(x + 24)
    return getfield(x, f)
end

function Base.getproperty(x::CUpti_ActivityGreenContext, f::Symbol)
    r = Ref{CUpti_ActivityGreenContext}(x)
    ptr = Base.unsafe_convert(Ptr{CUpti_ActivityGreenContext}, r)
    fptr = getproperty(ptr, f)
    GC.@preserve r unsafe_load(fptr)
end

function Base.setproperty!(x::Ptr{CUpti_ActivityGreenContext}, f::Symbol, v)
    return unsafe_store!(getproperty(x, f), v)
end

function Base.propertynames(x::CUpti_ActivityGreenContext, private::Bool=false)
    return (:kind, :contextId, :parentContextId, :deviceId, :numTpcs, :numMultiprocessors,
            :logicalTpcMaskSize, :padding, :logicalTpcMask, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
@cenum CUpti_ActivityGreenContextFieldIds::UInt32 begin
...
@checked function cuptiActivityEnable_v2(subscriber, kind, pActivityConfig)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiActivityEnable_v2(subscriber::CUpti_SubscriberHandle,
                                                  kind::CUpti_ActivityKind,
                                                  pActivityConfig::Ptr{CUpti_ActivityConfig})::CUptiResult
end

@checked function cuptiActivityDisable_v2(subscriber, kind, pActivityConfig)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiActivityDisable_v2(subscriber::CUpti_SubscriberHandle,
                                                   kind::CUpti_ActivityKind,
                                                   pActivityConfig::Ptr{CUpti_ActivityConfig})::CUptiResult
end

@checked function cuptiActivityEnableAndDump_v2(subscriber, kind, activityConfig)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiActivityEnableAndDump_v2(subscriber::CUpti_SubscriberHandle,
                                                         kind::CUpti_ActivityKind,
                                                         activityConfig::Ptr{CUpti_ActivityConfig})::CUptiResult
end

@checked function cuptiActivityGetEnabledKinds(subscriber, buffer, bufferSize,
                                               enabledKindsCount)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiActivityGetEnabledKinds(subscriber::CUpti_SubscriberHandle,
                                                        buffer::Ptr{CUpti_ActivityKind},
                                                        bufferSize::Ptr{UInt32},
                                                        enabledKindsCount::Ptr{UInt32})::CUptiResult
end

@checked function cuptiActivityGetStructSize(activityKind, version, activityStructSize)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiActivityGetStructSize(activityKind::CUpti_ActivityKind,
                                                      version::UInt32,
                                                      activityStructSize::Ptr{Csize_t})::CUptiResult
end

@checked function cuptiIsTracingSessionRunning(isRunning)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiIsTracingSessionRunning(isRunning::Ptr{UInt8})::CUptiResult
end
...
struct CUpti_BufferCallbackRequestInfo
    structSize::Csize_t
end

# typedef void ( CUPTIAPI * CUpti_BuffersCallbackRequestFunc_v2 ) ( uint8_t * * buffer , size_t * size , size_t * maxNumRecords , CUpti_BufferCallbackRequestInfo * pBufferRequestInfo )
const CUpti_BuffersCallbackRequestFunc_v2 = Ptr{Cvoid}

struct CUpti_BufferCallbackCompleteInfo
    structSize::Csize_t
    threadId::UInt64
    ppRecordLayouts::Ptr{Ptr{CUpti_ActivityRecordLayout}}
    numRecordLayouts::Csize_t
end

# typedef void ( CUPTIAPI * CUpti_BuffersCallbackCompleteFunc_v2 ) ( uint8_t * buffer , size_t size , size_t validSize , CUpti_BufferCallbackCompleteInfo * pBufferCompleteInfo )
const CUpti_BuffersCallbackCompleteFunc_v2 = Ptr{Cvoid}

@checked function cuptiActivityRegisterCallbacks_v2(subscriber, funcBufferRequested,
                                                    funcBufferCompleted)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiActivityRegisterCallbacks_v2(subscriber::CUpti_SubscriberHandle,
                                                             funcBufferRequested::CUpti_BuffersCallbackRequestFunc_v2,
                                                             funcBufferCompleted::CUpti_BuffersCallbackCompleteFunc_v2)::CUptiResult
end
...
@checked function cuptiActivityGetAttribute_v2(subscriber, attr, valueSize, value)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiActivityGetAttribute_v2(subscriber::CUpti_SubscriberHandle,
...
@checked function cuptiActivitySetAttribute_v2(subscriber, attr, valueSize, value)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiActivitySetAttribute_v2(subscriber::CUpti_SubscriberHandle,
                                                        attr::CUpti_ActivityAttribute,
                                                        valueSize::Ptr{Csize_t},
                                                        value::Ptr{Cvoid})::CUptiResult
end
...
@checked function cuptiActivityEnableCudaEventDeviceTimestamps(enable)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiActivityEnableCudaEventDeviceTimestamps(enable::UInt8)::CUptiResult
end
...
function Base.propertynames(x::CUpti_ActivityOverhead, private::Bool=false)
    return (:kind, :overheadKind, :objectKind, :objectId, :start, :_end,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivityOverhead2, private::Bool=false)
    return (:kind, :overheadKind, :objectKind, :objectId, :start, :_end, :correlationId,
            :reserved0, if private
                fieldnames(typeof(x))
            else
...
function Base.propertynames(x::CUpti_ActivityDevice, private::Bool=false)
    return (:kind, :flags, :globalMemoryBandwidth, :globalMemorySize, :constantMemorySize,
            :l2CacheSize, :numThreadsPerWarp, :coreClockRate, :numMemcpyEngines,
            :numMultiprocessors, :maxIPC, :maxWarpsPerMultiprocessor,
            :maxBlocksPerMultiprocessor, :maxRegistersPerBlock, :maxSharedMemoryPerBlock,
            :maxThreadsPerBlock, :maxBlockDimX, :maxBlockDimY, :maxBlockDimZ, :maxGridDimX,
            :maxGridDimY, :maxGridDimZ, :computeCapabilityMajor, :computeCapabilityMinor,
            :id, :pad, :name, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::CUpti_ActivityDevice2, private::Bool=false)
    return (:kind, :flags, :globalMemoryBandwidth, :globalMemorySize, :constantMemorySize,
            :l2CacheSize, :numThreadsPerWarp, :coreClockRate, :numMemcpyEngines,
            :numMultiprocessors, :maxIPC, :maxWarpsPerMultiprocessor,
            :maxBlocksPerMultiprocessor, :maxSharedMemoryPerMultiprocessor,
            ...
            :maxThreadsPerBlock, :maxBlockDimX, :maxBlockDimY, :maxBlockDimZ, :maxGridDimX,
            :maxGridDimY, :maxGridDimZ, :computeCapabilityMajor, :computeCapabilityMinor,
            :id, :eccEnabled, :uuid, :name, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::CUpti_ActivityDevice3, private::Bool=false)
    return (:kind, :flags, :globalMemoryBandwidth, :globalMemorySize, :constantMemorySize,
            :l2CacheSize, :numThreadsPerWarp, :coreClockRate, :numMemcpyEngines,
            :numMultiprocessors, :maxIPC, :maxWarpsPerMultiprocessor,
            :maxBlocksPerMultiprocessor, :maxSharedMemoryPerMultiprocessor,
            ...
            :maxThreadsPerBlock, :maxBlockDimX, :maxBlockDimY, :maxBlockDimZ, :maxGridDimX,
            :maxGridDimY, :maxGridDimZ, :computeCapabilityMajor, :computeCapabilityMinor,
            :id, :eccEnabled, :uuid, :name, :isCudaVisible, :reserved,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivityDevice4, private::Bool=false)
    return (:kind, :flags, :globalMemoryBandwidth, :globalMemorySize, :constantMemorySize,
            :l2CacheSize, :numThreadsPerWarp, :coreClockRate, :numMemcpyEngines,
            :numMultiprocessors, :maxIPC, :maxWarpsPerMultiprocessor,
            :maxBlocksPerMultiprocessor, :maxSharedMemoryPerMultiprocessor,
            ...
            :maxGridDimY, :maxGridDimZ, :computeCapabilityMajor, :computeCapabilityMinor,
            :id, :eccEnabled, :uuid, :name, :isCudaVisible, :isMigEnabled, :reserved,
            :gpuInstanceId, :computeInstanceId, :migUuid, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::CUpti_ActivityDevice5, private::Bool=false)
    return (:kind, :flags, :globalMemoryBandwidth, :globalMemorySize, :constantMemorySize,
            :l2CacheSize, :numThreadsPerWarp, :coreClockRate, :numMemcpyEngines,
            :numMultiprocessors, :maxIPC, :maxWarpsPerMultiprocessor,
            :maxBlocksPerMultiprocessor, :maxSharedMemoryPerMultiprocessor,
            ...
            :maxGridDimY, :maxGridDimZ, :computeCapabilityMajor, :computeCapabilityMinor,
            :id, :eccEnabled, :uuid, :name, :isCudaVisible, :isMigEnabled, :reserved,
            :gpuInstanceId, :computeInstanceId, :migUuid, :isNumaNode, :numaId,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivityKernel, private::Bool=false)
    return (:kind, :cacheConfigRequested, :cacheConfigExecuted, :registersPerThread, :start,
            :_end, :deviceId, :contextId, :streamId, :gridX, :gridY, :gridZ, :blockX,
            :blockY, :blockZ, :staticSharedMemory, :dynamicSharedMemory,
            :localMemoryPerThread, :localMemoryTotal, :correlationId, :runtimeCorrelationId,
            :pad, :name, :reserved0, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.getproperty(x::Ptr{var"##Ctag#366"}, f::Symbol)
    f === :both && return Ptr{UInt8}(x + 0)
    f === :config && return Ptr{var"##Ctag#367"}(x + 0)
    return getfield(x, f)
end
...
function Base.propertynames(x::var"##Ctag#366", private::Bool=false)
    return (:both, :config, if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.getproperty(x::Ptr{CUpti_ActivityKernel2}, f::Symbol)
    f === :kind && return Ptr{CUpti_ActivityKind}(x + 0)
    f === :cacheConfig && return Ptr{var"##Ctag#366"}(x + 4)
    f === :sharedMemoryConfig && return Ptr{UInt8}(x + 5)
    f === :registersPerThread && return Ptr{UInt16}(x + 6)
    f === :start && return Ptr{UInt64}(x + 8)
    f === :_end && return Ptr{UInt64}(x + 16)
...
function Base.propertynames(x::CUpti_ActivityKernel2, private::Bool=false)
    return (:kind, :cacheConfig, :sharedMemoryConfig, :registersPerThread, :start, :_end,
            :completed, :deviceId, :contextId, :streamId, :gridX, :gridY, :gridZ, :blockX,
            :blockY, :blockZ, :staticSharedMemory, :dynamicSharedMemory,
            :localMemoryPerThread, :localMemoryTotal, :correlationId, :gridId, :name,
            :reserved0, if private
                fieldnames(typeof(x))
...
function Base.getproperty(x::Ptr{var"##Ctag#289"}, f::Symbol)
    f === :both && return Ptr{UInt8}(x + 0)
    f === :config && return Ptr{var"##Ctag#290"}(x + 0)
    return getfield(x, f)
end
...
function Base.getproperty(x::Ptr{CUpti_ActivityKernel3}, f::Symbol)
    f === :kind && return Ptr{CUpti_ActivityKind}(x + 0)
    f === :cacheConfig && return Ptr{var"##Ctag#289"}(x + 4)
    f === :sharedMemoryConfig && return Ptr{UInt8}(x + 5)
    f === :registersPerThread && return Ptr{UInt16}(x + 6)
    f === :partitionedGlobalCacheRequested &&
        return Ptr{CUpti_ActivityPartitionedGlobalCacheConfig}(x + 8)
...
function Base.propertynames(x::CUpti_ActivityKernel3, private::Bool=false)
    return (:kind, :cacheConfig, :sharedMemoryConfig, :registersPerThread,
            :partitionedGlobalCacheRequested, :partitionedGlobalCacheExecuted, :start,
            :_end, :completed, :deviceId, :contextId, :streamId, :gridX, :gridY, :gridZ,
            :blockX, :blockY, :blockZ, :staticSharedMemory, :dynamicSharedMemory,
...
function Base.getproperty(x::Ptr{var"##Ctag#312"}, f::Symbol)
    f === :both && return Ptr{UInt8}(x + 0)
    f === :config && return Ptr{var"##Ctag#313"}(x + 0)
    return getfield(x, f)
end
...
function Base.propertynames(x::var"##Ctag#312", private::Bool=false)
    return (:both, :config, if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.getproperty(x::Ptr{CUpti_ActivityKernel4}, f::Symbol)
    f === :kind && return Ptr{CUpti_ActivityKind}(x + 0)
    f === :cacheConfig && return Ptr{var"##Ctag#312"}(x + 4)
    f === :sharedMemoryConfig && return Ptr{UInt8}(x + 5)
    f === :registersPerThread && return Ptr{UInt16}(x + 6)
    f === :partitionedGlobalCacheRequested &&
        return Ptr{CUpti_ActivityPartitionedGlobalCacheConfig}(x + 8)
...
function Base.propertynames(x::CUpti_ActivityKernel4, private::Bool=false)
    return (:kind, :cacheConfig, :sharedMemoryConfig, :registersPerThread,
            :partitionedGlobalCacheRequested, :partitionedGlobalCacheExecuted, :start,
            :_end, :completed, :deviceId, :contextId, :streamId, :gridX, :gridY, :gridZ,
            :blockX, :blockY, :blockZ, :staticSharedMemory, :dynamicSharedMemory,
            :localMemoryPerThread, :localMemoryTotal, :correlationId, :gridId, :name,
            :reserved0, :queued, :submitted, :launchType, :isSharedMemoryCarveoutRequested,
            :sharedMemoryCarveoutRequested, :padding, :sharedMemoryExecuted,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.getproperty(x::Ptr{var"##Ctag#346"}, f::Symbol)
    f === :both && return Ptr{UInt8}(x + 0)
    f === :config && return Ptr{var"##Ctag#347"}(x + 0)
    return getfield(x, f)
end
...
function Base.getproperty(x::Ptr{CUpti_ActivityKernel5}, f::Symbol)
    f === :kind && return Ptr{CUpti_ActivityKind}(x + 0)
    f === :cacheConfig && return Ptr{var"##Ctag#346"}(x + 4)
    f === :sharedMemoryConfig && return Ptr{UInt8}(x + 5)
    f === :registersPerThread && return Ptr{UInt16}(x + 6)
    f === :partitionedGlobalCacheRequested &&
        return Ptr{CUpti_ActivityPartitionedGlobalCacheConfig}(x + 8)
...
function Base.propertynames(x::CUpti_ActivityKernel5, private::Bool=false)
    return (:kind, :cacheConfig, :sharedMemoryConfig, :registersPerThread,
            :partitionedGlobalCacheRequested, :partitionedGlobalCacheExecuted, :start,
            :_end, :completed, :deviceId, :contextId, :streamId, :gridX, :gridY, :gridZ,
            :blockX, :blockY, :blockZ, :staticSharedMemory, :dynamicSharedMemory,
            ...
            :reserved0, :queued, :submitted, :launchType, :isSharedMemoryCarveoutRequested,
            :sharedMemoryCarveoutRequested, :padding, :sharedMemoryExecuted, :graphNodeId,
            :shmemLimitConfig, :graphId, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.getproperty(x::Ptr{var"##Ctag#293"}, f::Symbol)
    f === :both && return Ptr{UInt8}(x + 0)
    f === :config && return Ptr{var"##Ctag#294"}(x + 0)
    return getfield(x, f)
end
...
function Base.propertynames(x::var"##Ctag#293", private::Bool=false)
    return (:both, :config, if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.getproperty(x::Ptr{CUpti_ActivityKernel6}, f::Symbol)
    f === :kind && return Ptr{CUpti_ActivityKind}(x + 0)
    f === :cacheConfig && return Ptr{var"##Ctag#293"}(x + 4)
    f === :sharedMemoryConfig && return Ptr{UInt8}(x + 5)
    f === :registersPerThread && return Ptr{UInt16}(x + 6)
    f === :partitionedGlobalCacheRequested &&
        return Ptr{CUpti_ActivityPartitionedGlobalCacheConfig}(x + 8)
...
function Base.propertynames(x::CUpti_ActivityKernel6, private::Bool=false)
    return (:kind, :cacheConfig, :sharedMemoryConfig, :registersPerThread,
            :partitionedGlobalCacheRequested, :partitionedGlobalCacheExecuted, :start,
            :_end, :completed, :deviceId, :contextId, :streamId, :gridX, :gridY, :gridZ,
            :blockX, :blockY, :blockZ, :staticSharedMemory, :dynamicSharedMemory,
            ...
            :reserved0, :queued, :submitted, :launchType, :isSharedMemoryCarveoutRequested,
            :sharedMemoryCarveoutRequested, :padding, :sharedMemoryExecuted, :graphNodeId,
            :shmemLimitConfig, :graphId, :pAccessPolicyWindow, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.getproperty(x::Ptr{var"##Ctag#296"}, f::Symbol)
    f === :both && return Ptr{UInt8}(x + 0)
    f === :config && return Ptr{var"##Ctag#297"}(x + 0)
    return getfield(x, f)
end
...
function Base.getproperty(x::Ptr{CUpti_ActivityKernel7}, f::Symbol)
    f === :kind && return Ptr{CUpti_ActivityKind}(x + 0)
    f === :cacheConfig && return Ptr{var"##Ctag#296"}(x + 4)
    f === :sharedMemoryConfig && return Ptr{UInt8}(x + 5)
    f === :registersPerThread && return Ptr{UInt16}(x + 6)
    f === :partitionedGlobalCacheRequested &&
        return Ptr{CUpti_ActivityPartitionedGlobalCacheConfig}(x + 8)
...
function Base.propertynames(x::CUpti_ActivityKernel7, private::Bool=false)
    return (:kind, :cacheConfig, :sharedMemoryConfig, :registersPerThread,
            :partitionedGlobalCacheRequested, :partitionedGlobalCacheExecuted, :start,
            :_end, :completed, :deviceId, :contextId, :streamId, :gridX, :gridY, :gridZ,
            :blockX, :blockY, :blockZ, :staticSharedMemory, :dynamicSharedMemory,
...
function Base.getproperty(x::Ptr{var"##Ctag#380"}, f::Symbol)
    f === :both && return Ptr{UInt8}(x + 0)
    f === :config && return Ptr{var"##Ctag#381"}(x + 0)
    return getfield(x, f)
end
...
function Base.propertynames(x::var"##Ctag#380", private::Bool=false)
    return (:both, :config, if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.getproperty(x::Ptr{CUpti_ActivityKernel8}, f::Symbol)
    f === :kind && return Ptr{CUpti_ActivityKind}(x + 0)
    f === :cacheConfig && return Ptr{var"##Ctag#380"}(x + 4)
    f === :sharedMemoryConfig && return Ptr{UInt8}(x + 5)
    f === :registersPerThread && return Ptr{UInt16}(x + 6)
    f === :partitionedGlobalCacheRequested &&
        return Ptr{CUpti_ActivityPartitionedGlobalCacheConfig}(x + 8)
...
function Base.propertynames(x::CUpti_ActivityKernel8, private::Bool=false)
    return (:kind, :cacheConfig, :sharedMemoryConfig, :registersPerThread,
            :partitionedGlobalCacheRequested, :partitionedGlobalCacheExecuted, :start,
            :_end, :completed, :deviceId, :contextId, :streamId, :gridX, :gridY, :gridZ,
            :blockX, :blockY, :blockZ, :staticSharedMemory, :dynamicSharedMemory,
...
function Base.getproperty(x::Ptr{var"##Ctag#355"}, f::Symbol)
    f === :both && return Ptr{UInt8}(x + 0)
    f === :config && return Ptr{var"##Ctag#356"}(x + 0)
    return getfield(x, f)
end
...
function Base.getproperty(x::Ptr{CUpti_ActivityKernel9}, f::Symbol)
    f === :kind && return Ptr{CUpti_ActivityKind}(x + 0)
    f === :cacheConfig && return Ptr{var"##Ctag#355"}(x + 4)
    f === :sharedMemoryConfig && return Ptr{UInt8}(x + 5)
    f === :registersPerThread && return Ptr{UInt16}(x + 6)
    f === :partitionedGlobalCacheRequested &&
        return Ptr{CUpti_ActivityPartitionedGlobalCacheConfig}(x + 8)
...
function Base.propertynames(x::CUpti_ActivityKernel9, private::Bool=false)
    return (:kind, :cacheConfig, :sharedMemoryConfig, :registersPerThread,
            :partitionedGlobalCacheRequested, :partitionedGlobalCacheExecuted, :start,
            :_end, :completed, :deviceId, :contextId, :streamId, :gridX, :gridY, :gridZ,
            :blockX, :blockY, :blockZ, :staticSharedMemory, :dynamicSharedMemory,
            ...
            :shmemLimitConfig, :graphId, :pAccessPolicyWindow, :channelID, :channelType,
            :clusterX, :clusterY, :clusterZ, :clusterSchedulingPolicy, :localMemoryTotal_v2,
            :maxPotentialClusterSize, :maxActiveClusters, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.getproperty(x::Ptr{var"##Ctag#348"}, f::Symbol)
    f === :both && return Ptr{UInt8}(x + 0)
    f === :config && return Ptr{var"##Ctag#349"}(x + 0)
    return getfield(x, f)
end
...
function Base.propertynames(x::var"##Ctag#348", private::Bool=false)
    return (:both, :config, if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.getproperty(x::Ptr{CUpti_ActivityKernel10}, f::Symbol)
    f === :kind && return Ptr{CUpti_ActivityKind}(x + 0)
    f === :cacheConfig && return Ptr{var"##Ctag#348"}(x + 4)
    f === :sharedMemoryConfig && return Ptr{UInt8}(x + 5)
    f === :registersPerThread && return Ptr{UInt16}(x + 6)
    f === :partitionedGlobalCacheRequested &&
        return Ptr{CUpti_ActivityPartitionedGlobalCacheConfig}(x + 8)
...
function Base.getproperty(x::CUpti_ActivityKernel10, f::Symbol)
    r = Ref{CUpti_ActivityKernel10}(x)
    ptr = Base.unsafe_convert(Ptr{CUpti_ActivityKernel10}, r)
    fptr = getproperty(ptr, f)
    GC.@preserve r unsafe_load(fptr)
end

function Base.setproperty!(x::Ptr{CUpti_ActivityKernel10}, f::Symbol, v)
    return unsafe_store!(getproperty(x, f), v)
end

function Base.propertynames(x::CUpti_ActivityKernel10, private::Bool=false)
    return (:kind, :cacheConfig, :sharedMemoryConfig, :registersPerThread,
            :partitionedGlobalCacheRequested, :partitionedGlobalCacheExecuted, :start,
            :_end, :completed, :deviceId, :contextId, :streamId, :gridX, :gridY, :gridZ,
            :blockX, :blockY, :blockZ, :staticSharedMemory, :dynamicSharedMemory,
...
function Base.propertynames(x::CUpti_ActivityMemcpy, private::Bool=false)
    return (:kind, :copyKind, :srcKind, :dstKind, :flags, :bytes, :start, :_end, :deviceId,
            :contextId, :streamId, :correlationId, :runtimeCorrelationId, :pad, :reserved0,
            if private
                fieldnames(typeof(x))
...
function Base.propertynames(x::CUpti_ActivityMemcpy3, private::Bool=false)
    return (:kind, :copyKind, :srcKind, :dstKind, :flags, :bytes, :start, :_end, :deviceId,
            :contextId, :streamId, :correlationId, :runtimeCorrelationId, :pad, :reserved0,
            :graphNodeId, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::CUpti_ActivityMemcpy4, private::Bool=false)
    return (:kind, :copyKind, :srcKind, :dstKind, :flags, :bytes, :start, :_end, :deviceId,
            :contextId, :streamId, :correlationId, :runtimeCorrelationId, :pad, :reserved0,
            :graphNodeId, :graphId, :padding, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::CUpti_ActivityMemcpyPtoP, private::Bool=false)
    return (:kind, :copyKind, :srcKind, :dstKind, :flags, :bytes, :start, :_end, :deviceId,
            :contextId, :streamId, :srcDeviceId, :srcContextId, :dstDeviceId, :dstContextId,
            :correlationId, :reserved0, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::CUpti_ActivityMemcpyPtoP2, private::Bool=false)
    return (:kind, :copyKind, :srcKind, :dstKind, :flags, :bytes, :start, :_end, :deviceId,
            :contextId, :streamId, :srcDeviceId, :srcContextId, :dstDeviceId, :dstContextId,
            :correlationId, :reserved0, :graphNodeId, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::CUpti_ActivityMemcpyPtoP3, private::Bool=false)
    return (:kind, :copyKind, :srcKind, :dstKind, :flags, :bytes, :start, :_end, :deviceId,
            :contextId, :streamId, :srcDeviceId, :srcContextId, :dstDeviceId, :dstContextId,
            :correlationId, :reserved0, :graphNodeId, :graphId, :padding,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivityMemset, private::Bool=false)
    return (:kind, :value, :bytes, :start, :_end, :deviceId, :contextId, :streamId,
            :correlationId, :flags, :memoryKind, :pad, :reserved0,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivityMemset2, private::Bool=false)
    return (:kind, :value, :bytes, :start, :_end, :deviceId, :contextId, :streamId,
            :correlationId, :flags, :memoryKind, :pad, :reserved0, :graphNodeId,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivityMemset3, private::Bool=false)
    return (:kind, :value, :bytes, :start, :_end, :deviceId, :contextId, :streamId,
            :correlationId, :flags, :memoryKind, :pad, :reserved0, :graphNodeId, :graphId,
            :padding, if private
                fieldnames(typeof(x))
...
function Base.getproperty(x::Ptr{var"##Ctag#332"}, f::Symbol)
    ...
    f === :address && return Ptr{UInt64}(x + 8)
    f === :releaseThreshold && return Ptr{UInt64}(x + 16)
    f === :pool && return Ptr{var"##Ctag#333"}(x + 24)
    return getfield(x, f)
end
...
function Base.propertynames(x::CUpti_ActivityMemory2, private::Bool=false)
    return (:kind, :memoryOperationType, :memoryKind, :correlationId, :address, :bytes,
            :timestamp, :PC, :processId, :deviceId, :contextId, :streamId, :name, :isAsync,
            :pad1, :memoryPoolConfig, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::var"##Ctag#317", private::Bool=false)
    return (:size, :processId, if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.getproperty(x::Ptr{var"##Ctag#316"}, f::Symbol)
    ...
    f === :address && return Ptr{UInt64}(x + 8)
    f === :releaseThreshold && return Ptr{UInt64}(x + 16)
    f === :pool && return Ptr{var"##Ctag#317"}(x + 24)
    f === :utilizedSize && return Ptr{UInt64}(x + 32)
    return getfield(x, f)
end
...
function Base.propertynames(x::CUpti_ActivityMemory3, private::Bool=false)
    return (:kind, :memoryOperationType, :memoryKind, :correlationId, :address, :bytes,
            :timestamp, :PC, :processId, :deviceId, :contextId, :streamId, :name, :isAsync,
            :pad1, :memoryPoolConfig, if private
                fieldnames(typeof(x))
...
function Base.propertynames(x::CUpti_ActivityMemoryPool, private::Bool=false)
    return (:kind, :memoryPoolOperationType, :memoryPoolType, :correlationId, :processId,
            :deviceId, :minBytesToKeep, :address, :size, :releaseThreshold, :timestamp,
            if private
                fieldnames(typeof(x))
...
function Base.propertynames(x::CUpti_ActivityMemoryPool2, private::Bool=false)
    return (:kind, :memoryPoolOperationType, :memoryPoolType, :correlationId, :processId,
            :deviceId, :minBytesToKeep, :address, :size, :releaseThreshold, :timestamp,
            :utilizedSize, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::CUpti_ActivityMarker, private::Bool=false)
    return (:kind, :flags, :timestamp, :id, :objectKind, :objectId, :pad, :name,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivityGlobalAccess, private::Bool=false)
    return (:kind, :flags, :sourceLocatorId, :correlationId, :pcOffset, :executed,
            :threadsExecuted, :l2_transactions, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::CUpti_ActivityGlobalAccess2, private::Bool=false)
    return (:kind, :flags, :sourceLocatorId, :correlationId, :functionId, :pcOffset,
            :threadsExecuted, :l2_transactions, :theoreticalL2Transactions, :executed, :pad,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivityBranch, private::Bool=false)
    return (:kind, :sourceLocatorId, :correlationId, :pcOffset, :executed, :diverged,
            :threadsExecuted, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::CUpti_ActivityPCSampling, private::Bool=false)
    return (:kind, :flags, :sourceLocatorId, :correlationId, :functionId, :pcOffset,
            :samples, :stallReason, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::CUpti_ActivityPCSampling2, private::Bool=false)
    return (:kind, :flags, :sourceLocatorId, :correlationId, :functionId, :pcOffset,
            :latencySamples, :samples, :stallReason, :pad, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::CUpti_ActivityUnifiedMemoryCounter, private::Bool=false)
    return (:kind, :counterKind, :scope, :deviceId, :value, :timestamp, :processId, :pad,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivityUnifiedMemoryCounter2, private::Bool=false)
    return (:kind, :counterKind, :value, :start, :_end, :address, :srcId, :dstId, :streamId,
            :processId, :flags, :pad, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.getproperty(x::Ptr{var"##Ctag#277"}, f::Symbol)
    f === :uuidDev && return Ptr{CUuuid}(x + 0)
    f === :npu && return Ptr{var"##Ctag#278"}(x + 0)
    return getfield(x, f)
end
...
function Base.getproperty(x::Ptr{var"##Ctag#279"}, f::Symbol)
    f === :uuidDev && return Ptr{CUuuid}(x + 0)
    f === :npu && return Ptr{var"##Ctag#280"}(x + 0)
    return getfield(x, f)
end
...
function Base.getproperty(x::Ptr{CUpti_ActivityNvLink}, f::Symbol)
    ...
    f === :typeDev0 && return Ptr{CUpti_DevType}(x + 8)
    f === :typeDev1 && return Ptr{CUpti_DevType}(x + 12)
    f === :idDev0 && return Ptr{var"##Ctag#277"}(x + 16)
    f === :idDev1 && return Ptr{var"##Ctag#279"}(x + 32)
    f === :flag && return Ptr{UInt32}(x + 48)
    f === :physicalNvLinkCount && return Ptr{UInt32}(x + 52)
...
function Base.propertynames(x::CUpti_ActivityNvLink, private::Bool=false)
    return (:kind, :nvlinkVersion, :typeDev0, :typeDev1, :idDev0, :idDev1, :flag,
            :physicalNvLinkCount, :portDev0, :portDev1, :bandwidth,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.getproperty(x::Ptr{var"##Ctag#304"}, f::Symbol)
    f === :uuidDev && return Ptr{CUuuid}(x + 0)
    f === :npu && return Ptr{var"##Ctag#305"}(x + 0)
    return getfield(x, f)
end
...
function Base.getproperty(x::Ptr{var"##Ctag#306"}, f::Symbol)
    f === :uuidDev && return Ptr{CUuuid}(x + 0)
    f === :npu && return Ptr{var"##Ctag#307"}(x + 0)
    return getfield(x, f)
end
...
function Base.getproperty(x::Ptr{CUpti_ActivityNvLink2}, f::Symbol)
    ...
    f === :typeDev0 && return Ptr{CUpti_DevType}(x + 8)
    f === :typeDev1 && return Ptr{CUpti_DevType}(x + 12)
    f === :idDev0 && return Ptr{var"##Ctag#304"}(x + 16)
    f === :idDev1 && return Ptr{var"##Ctag#306"}(x + 32)
    f === :flag && return Ptr{UInt32}(x + 48)
    f === :physicalNvLinkCount && return Ptr{UInt32}(x + 52)
    f === :portDev0 && return Ptr{NTuple{16,Int8}}(x + 56)
    f === :portDev1 && return Ptr{NTuple{16,Int8}}(x + 72)
    f === :bandwidth && return Ptr{UInt64}(x + 88)
    return getfield(x, f)
end
...
function Base.propertynames(x::CUpti_ActivityNvLink2, private::Bool=false)
    return (:kind, :nvlinkVersion, :typeDev0, :typeDev1, :idDev0, :idDev1, :flag,
            :physicalNvLinkCount, :portDev0, :portDev1, :bandwidth,
            if private
                fieldnames(typeof(x))
...
function Base.getproperty(x::Ptr{var"##Ctag#298"}, f::Symbol)
    f === :uuidDev && return Ptr{CUuuid}(x + 0)
    f === :npu && return Ptr{var"##Ctag#299"}(x + 0)
    return getfield(x, f)
end
...
function Base.getproperty(x::Ptr{var"##Ctag#300"}, f::Symbol)
    f === :uuidDev && return Ptr{CUuuid}(x + 0)
    f === :npu && return Ptr{var"##Ctag#301"}(x + 0)
    return getfield(x, f)
end
...
function Base.propertynames(x::var"##Ctag#300", private::Bool=false)
    return (:uuidDev, :npu, if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.getproperty(x::Ptr{CUpti_ActivityNvLink3}, f::Symbol)
    ...
    f === :typeDev0 && return Ptr{CUpti_DevType}(x + 8)
    f === :typeDev1 && return Ptr{CUpti_DevType}(x + 12)
    f === :idDev0 && return Ptr{var"##Ctag#298"}(x + 16)
    f === :idDev1 && return Ptr{var"##Ctag#300"}(x + 32)
    f === :flag && return Ptr{UInt32}(x + 48)
    f === :physicalNvLinkCount && return Ptr{UInt32}(x + 52)
    f === :portDev0 && return Ptr{NTuple{16,Int8}}(x + 56)
    f === :portDev1 && return Ptr{NTuple{16,Int8}}(x + 72)
    f === :bandwidth && return Ptr{UInt64}(x + 88)
    f === :nvswitchConnected && return Ptr{UInt8}(x + 96)
    f === :pad && return Ptr{NTuple{7,UInt8}}(x + 97)
    return getfield(x, f)
end
...
function Base.propertynames(x::CUpti_ActivityNvLink3, private::Bool=false)
    return (:kind, :nvlinkVersion, :typeDev0, :typeDev1, :idDev0, :idDev1, :flag,
            :physicalNvLinkCount, :portDev0, :portDev1, :bandwidth, :nvswitchConnected,
            :pad, if private
                fieldnames(typeof(x))
...
function Base.propertynames(x::CUpti_ActivityNvLink4, private::Bool=false)
    return (:kind, :nvlinkVersion, :typeDev0, :typeDev1, :idDev0, :idDev1, :flag,
            :physicalNvLinkCount, :portDev0, :portDev1, :bandwidth, :nvswitchConnected,
            :pad, if private
                fieldnames(typeof(x))
...
function Base.propertynames(x::CUpti_ActivityContext, private::Bool=false)
    return (:kind, :contextId, :deviceId, :computeApiKind, :nullStreamId,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivityContext2, private::Bool=false)
    return (:kind, :contextId, :deviceId, :computeApiKind, :nullStreamId, :parentContextId,
            :isGreenContext, :padding, :numMultiprocessors, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::CUpti_ActivityContext3, private::Bool=false)
    return (:kind, :contextId, :deviceId, :computeApiKind, :nullStreamId, :parentContextId,
            :isGreenContext, :padding, :numMultiprocessors, :cigMode, :padding2,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivityJit, private::Bool=false)
    return (:kind, :jitEntryType, :jitOperationType, :deviceId, :start, :_end,
            :correlationId, :padding, :jitOperationCorrelationId, :cacheSize, :cachePath,
            if private
                fieldnames(typeof(x))
...
function Base.propertynames(x::CUpti_ActivityCudaEvent, private::Bool=false)
    return (:kind, :correlationId, :contextId, :streamId, :eventId, :pad,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivitySynchronization, private::Bool=false)
    return (:kind, :type, :start, :_end, :correlationId, :contextId, :streamId,
            :cudaEventId, if private
                fieldnames(typeof(x))
            else
                ()
            end...)
...
function Base.propertynames(x::CUpti_ActivityMemcpy5, private::Bool=false)
    return (:kind, :copyKind, :srcKind, :dstKind, :flags, :bytes, :start, :_end, :deviceId,
            :contextId, :streamId, :correlationId, :runtimeCorrelationId, :pad, :reserved0,
            :graphNodeId, :graphId, :channelID, :channelType, :pad2,
            if private
                fieldnames(typeof(x))
            else
                ()
...
function Base.propertynames(x::CUpti_ActivityMarkerData, private::Bool=false)
    return (:kind, :flags, :id, :payloadKind, :payload, :color, :category,
            if private
                fieldnames(typeof(x))
            else
                ()
...
struct CUpti_Profiler_GetCounterAvailability_Params
    ...
    bAllowDeviceLevelCounters::Cint
end
...
struct CUpti_Profiler_DeviceSupported_Params
    ...
    sku::CUpti_Profiler_Support_Level
end
...
@cenum CUpti_ProfilerType::UInt32 begin
    CUPTI_PROFILER_TYPE_RANGE_PROFILER = 0
    CUPTI_PROFILER_TYPE_PM_SAMPLING = 1
    CUPTI_PROFILER_TYPE_PROFILER_INVALID = 2
end

mutable struct CUpti_Profiler_Host_Object end

struct CUpti_Profiler_Host_Initialize_Params
    structSize::Csize_t
    pPriv::Ptr{Cvoid}
    profilerType::CUpti_ProfilerType
    pChipName::Cstring
    pCounterAvailabilityImage::Ptr{UInt8}
    pHostObject::Ptr{CUpti_Profiler_Host_Object}
    pSinglePassMetricSetName::Cstring
end
...
@cenum CUpti_MetricType::UInt32 begin
...
struct CUpti_Profiler_Host_GetSubMetrics_Params
    ...
    metricType::CUpti_MetricType
    pMetricName::Cstring
    numOfSubmetrics::Csize_t
    ppSubMetrics::Ptr{Cstring}
end

@cenum CUpti_MetricCollectionScope::UInt32 begin
    CUPTI_METRIC_COLLECTION_SCOPE_CONTEXT = 0
    CUPTI_METRIC_COLLECTION_SCOPE_DEVICE = 1
    CUPTI_METRIC_COLLECTION_SCOPE_INVALID = 2
end
...
struct CUpti_Profiler_Host_GetSinglePassSets_Params
    ...
    pPriv::Ptr{Cvoid}
    pChipName::Cstring
    numOfSinglePassSets::Csize_t
    ppSinglePassSets::Ptr{Cstring}
end
...
struct CUpti_Profiler_Host_GetMetricsInSinglePassSet_Params
    ...
    pPriv::Ptr{Cvoid}
    pChipName::Cstring
    pSinglePassSetName::Cstring
    numOfMetricsInSinglePassSet::Csize_t
    metricsBufferSize::Csize_t
    pMetricsIndicesBuffer::Ptr{Csize_t}
    pMetricsBuffer::Ptr{UInt8}
end

@checked function cuptiProfilerHostInitialize(pParams)
    @gcsafe_ccall libcupti.cuptiProfilerHostInitialize(pParams::Ptr{CUpti_Profiler_Host_Initialize_Params})::CUptiResult
end

@checked function cuptiProfilerHostDeinitialize(pParams)
    @gcsafe_ccall libcupti.cuptiProfilerHostDeinitialize(pParams::Ptr{CUpti_Profiler_Host_Deinitialize_Params})::CUptiResult
end

@checked function cuptiProfilerHostGetSupportedChips(pParams)
    @gcsafe_ccall libcupti.cuptiProfilerHostGetSupportedChips(pParams::Ptr{CUpti_Profiler_Host_GetSupportedChips_Params})::CUptiResult
end

@checked function cuptiProfilerHostGetBaseMetrics(pParams)
    @gcsafe_ccall libcupti.cuptiProfilerHostGetBaseMetrics(pParams::Ptr{CUpti_Profiler_Host_GetBaseMetrics_Params})::CUptiResult
end

@checked function cuptiProfilerHostGetSubMetrics(pParams)
    @gcsafe_ccall libcupti.cuptiProfilerHostGetSubMetrics(pParams::Ptr{CUpti_Profiler_Host_GetSubMetrics_Params})::CUptiResult
end

@checked function cuptiProfilerHostGetMetricProperties(pParams)
    @gcsafe_ccall libcupti.cuptiProfilerHostGetMetricProperties(pParams::Ptr{CUpti_Profiler_Host_GetMetricProperties_Params})::CUptiResult
end

@checked function cuptiProfilerHostGetRangeName(pParams)
    @gcsafe_ccall libcupti.cuptiProfilerHostGetRangeName(pParams::Ptr{CUpti_Profiler_Host_GetRangeName_Params})::CUptiResult
end

@checked function cuptiProfilerHostEvaluateToGpuValues(pParams)
    @gcsafe_ccall libcupti.cuptiProfilerHostEvaluateToGpuValues(pParams::Ptr{CUpti_Profiler_Host_EvaluateToGpuValues_Params})::CUptiResult
end

@checked function cuptiProfilerHostConfigAddMetrics(pParams)
    @gcsafe_ccall libcupti.cuptiProfilerHostConfigAddMetrics(pParams::Ptr{CUpti_Profiler_Host_ConfigAddMetrics_Params})::CUptiResult
end

@checked function cuptiProfilerHostGetConfigImageSize(pParams)
    @gcsafe_ccall libcupti.cuptiProfilerHostGetConfigImageSize(pParams::Ptr{CUpti_Profiler_Host_GetConfigImageSize_Params})::CUptiResult
end

@checked function cuptiProfilerHostGetConfigImage(pParams)
    @gcsafe_ccall libcupti.cuptiProfilerHostGetConfigImage(pParams::Ptr{CUpti_Profiler_Host_GetConfigImage_Params})::CUptiResult
end

@checked function cuptiProfilerHostGetNumOfPasses(pParams)
    @gcsafe_ccall libcupti.cuptiProfilerHostGetNumOfPasses(pParams::Ptr{CUpti_Profiler_Host_GetNumOfPasses_Params})::CUptiResult
end

@checked function cuptiProfilerHostGetMaxNumHardwareMetricsPerPass(pParams)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiProfilerHostGetMaxNumHardwareMetricsPerPass(pParams::Ptr{CUpti_Profiler_Host_GetMaxNumHardwareMetricsPerPass_Params})::CUptiResult
end

@checked function cuptiProfilerHostGetSinglePassSets(pParams)
    @gcsafe_ccall libcupti.cuptiProfilerHostGetSinglePassSets(pParams::Ptr{CUpti_Profiler_Host_GetSinglePassSets_Params})::CUptiResult
end

@checked function cuptiProfilerHostGetMetricsInSinglePassSet(pParams)
    @gcsafe_ccall libcupti.cuptiProfilerHostGetMetricsInSinglePassSet(pParams::Ptr{CUpti_Profiler_Host_GetMetricsInSinglePassSet_Params})::CUptiResult
end

mutable struct CUpti_RangeProfiler_Object end

struct CUpti_RangeProfiler_SetConfig_Params
    structSize::Csize_t
    pPriv::Ptr{Cvoid}
    pRangeProfilerObject::Ptr{CUpti_RangeProfiler_Object}
    configSize::Csize_t
    pConfig::Ptr{UInt8}
    counterDataImageSize::Csize_t
    pCounterDataImage::Ptr{UInt8}
...
struct CUpti_RangeProfiler_Stop_Params
    ...
    pRangeProfilerObject::Ptr{CUpti_RangeProfiler_Object}
    passIndex::Csize_t
    targetNestingLevel::Csize_t
    isAllPassSubmitted::UInt8
end
...
struct CUpti_RangeProfiler_GetCounterDataSize_Params
    ...
    pPriv::Ptr{Cvoid}
    pRangeProfilerObject::Ptr{CUpti_RangeProfiler_Object}
    pMetricNames::Ptr{Cstring}
    numMetrics::Csize_t
    maxNumOfRanges::Csize_t
    maxNumRangeTreeNodes::UInt32
    counterDataSize::Csize_t
end
...
struct CUpti_RangeProfiler_CounterData_GetRangeInfo_Params
    ...
    counterDataImageSize::Csize_t
    rangeIndex::Csize_t
    rangeDelimiter::Cstring
    rangeName::Cstring
end

@checked function cuptiRangeProfilerSetConfig(pParams)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiRangeProfilerSetConfig(pParams::Ptr{CUpti_RangeProfiler_SetConfig_Params})::CUptiResult
end

@checked function cuptiRangeProfilerEnable(pParams)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiRangeProfilerEnable(pParams::Ptr{CUpti_RangeProfiler_Enable_Params})::CUptiResult
end

@checked function cuptiRangeProfilerDisable(pParams)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiRangeProfilerDisable(pParams::Ptr{CUpti_RangeProfiler_Disable_Params})::CUptiResult
end

@checked function cuptiRangeProfilerStart(pParams)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiRangeProfilerStart(pParams::Ptr{CUpti_RangeProfiler_Start_Params})::CUptiResult
end

@checked function cuptiRangeProfilerStop(pParams)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiRangeProfilerStop(pParams::Ptr{CUpti_RangeProfiler_Stop_Params})::CUptiResult
end

@checked function cuptiRangeProfilerPushRange(pParams)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiRangeProfilerPushRange(pParams::Ptr{CUpti_RangeProfiler_PushRange_Params})::CUptiResult
end

@checked function cuptiRangeProfilerPopRange(pParams)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiRangeProfilerPopRange(pParams::Ptr{CUpti_RangeProfiler_PopRange_Params})::CUptiResult
end

@checked function cuptiRangeProfilerDecodeData(pParams)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiRangeProfilerDecodeData(pParams::Ptr{CUpti_RangeProfiler_DecodeData_Params})::CUptiResult
end

@checked function cuptiRangeProfilerGetCounterDataSize(pParams)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiRangeProfilerGetCounterDataSize(pParams::Ptr{CUpti_RangeProfiler_GetCounterDataSize_Params})::CUptiResult
end

@checked function cuptiRangeProfilerCounterDataImageInitialize(pParams)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiRangeProfilerCounterDataImageInitialize(pParams::Ptr{CUpti_RangeProfiler_CounterDataImage_Initialize_Params})::CUptiResult
end

@checked function cuptiRangeProfilerGetCounterDataInfo(pParams)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiRangeProfilerGetCounterDataInfo(pParams::Ptr{CUpti_RangeProfiler_GetCounterDataInfo_Params})::CUptiResult
end

@checked function cuptiRangeProfilerCounterDataGetRangeInfo(pParams)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiRangeProfilerCounterDataGetRangeInfo(pParams::Ptr{CUpti_RangeProfiler_CounterData_GetRangeInfo_Params})::CUptiResult
end

mutable struct CUpti_PmSampling_Object end

@cenum CUpti_PmSampling_TriggerMode::UInt32 begin
...
@cenum CUpti_PmSampling_HardwareBuffer_AppendMode::UInt32 begin
...
struct CUpti_PmSampling_SetConfig_Params
    structSize::Csize_t
    pPriv::Ptr{Cvoid}
    pPmSamplingObject::Ptr{CUpti_PmSampling_Object}
    configSize::Csize_t
    pConfig::Ptr{UInt8}
    hardwareBufferSize::Csize_t
    samplingInterval::UInt64
    triggerMode::CUpti_PmSampling_TriggerMode
...
@cenum CUpti_PmSampling_DecodeStopReason::UInt32 begin
...
struct CUpti_PmSampling_DecodeData_Params
    ...
    pCounterDataImage::Ptr{UInt8}
    counterDataImageSize::Csize_t
    decodeStopReason::CUpti_PmSampling_DecodeStopReason
    overflow::UInt8
end
...
struct CUpti_PmSampling_GetCounterDataInfo_Params
    ...
    pCounterDataImage::Ptr{UInt8}
    counterDataImageSize::Csize_t
    numTotalSamples::Csize_t
    numPopulatedSamples::Csize_t
    numCompletedSamples::Csize_t
end
...
@checked function cuptiPmSamplingSetConfig(pParams)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiPmSamplingSetConfig(pParams::Ptr{CUpti_PmSampling_SetConfig_Params})::CUptiResult
end

@checked function cuptiPmSamplingEnable(pParams)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiPmSamplingEnable(pParams::Ptr{CUpti_PmSampling_Enable_Params})::CUptiResult
end

@checked function cuptiPmSamplingDisable(pParams)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiPmSamplingDisable(pParams::Ptr{CUpti_PmSampling_Disable_Params})::CUptiResult
end

@checked function cuptiPmSamplingStart(pParams)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiPmSamplingStart(pParams::Ptr{CUpti_PmSampling_Start_Params})::CUptiResult
end

@checked function cuptiPmSamplingStop(pParams)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiPmSamplingStop(pParams::Ptr{CUpti_PmSampling_Stop_Params})::CUptiResult
end

@checked function cuptiPmSamplingDecodeData(pParams)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiPmSamplingDecodeData(pParams::Ptr{CUpti_PmSampling_DecodeData_Params})::CUptiResult
end

@checked function cuptiPmSamplingGetCounterAvailability(pParams)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiPmSamplingGetCounterAvailability(pParams::Ptr{CUpti_PmSampling_GetCounterAvailability_Params})::CUptiResult
end

@checked function cuptiPmSamplingGetCounterDataSize(pParams)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiPmSamplingGetCounterDataSize(pParams::Ptr{CUpti_PmSampling_GetCounterDataSize_Params})::CUptiResult
end

@checked function cuptiPmSamplingCounterDataImageInitialize(pParams)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiPmSamplingCounterDataImageInitialize(pParams::Ptr{CUpti_PmSampling_CounterDataImage_Initialize_Params})::CUptiResult
end

@checked function cuptiPmSamplingGetCounterDataInfo(pParams)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiPmSamplingGetCounterDataInfo(pParams::Ptr{CUpti_PmSampling_GetCounterDataInfo_Params})::CUptiResult
end

@checked function cuptiPmSamplingCounterDataGetSampleInfo(pParams)
    initialize_context()
    @gcsafe_ccall libcupti.cuptiPmSamplingCounterDataGetSampleInfo(pParams::Ptr{CUpti_PmSampling_CounterData_GetSampleInfo_Params})::CUptiResult
end
...
const CUPTI_SUBSCRIBER_NAME_MAX_LEN = 53

const CUPTI_OLD_SUBSCRIBER_NAME_MIN_LEN = 64

const CUpti_SubscriberParams_STRUCT_SIZE = @CUPTI_CALLBACK_STRUCT_SIZE(CUpti_SubscriberParams,
                                                                       padding)
...
const CUPTI_MAX_DEVICES = 256

const CUPTI_UNIFIED_MEMORY_CPU_DEVICE_ID = uint32_t(Cuint(0xffffffff))

const CUPTI_INVALID_DEVICE_ID = uint32_t(Cuint(0xffffffff))

const CUPTI_INVALID_CONTEXT_ID = 0

const CUPTI_INVALID_STREAM_ID = 0
...
const CUPTI_NVLINK_INVALID_PORT = uint32_t(Cuint(0xffffffff))

const CUPTI_DECOMPRESSED_BYTES_UNKNOWN = Clonglong(0)

const CUPTI_MAX_GPUS = 32

const CUpti_ActivityConfig_STRUCT_SIZE = @CUPTI_ACTIVITY_STRUCT_SIZE(CUpti_ActivityConfig,
                                                                     fieldSelection)

const CUPTI_MAX_NVLINK_PORTS = 256
...
const CUpti_Profiler_Host_Initialize_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_Profiler_Host_Initialize_Params,
                                                                                      pSinglePassMetricSetName)

const CUpti_Profiler_Host_Deinitialize_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_Profiler_Host_Deinitialize_Params,
                                                                                        pHostObject)

const CUpti_Profiler_Host_GetSupportedChips_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_Profiler_Host_GetSupportedChips_Params,
                                                                                             ppChipNames)

const CUpti_Profiler_Host_GetBaseMetrics_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_Profiler_Host_GetBaseMetrics_Params,
                                                                                          numMetrics)

const CUpti_Profiler_Host_GetSubMetrics_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_Profiler_Host_GetSubMetrics_Params,
                                                                                         ppSubMetrics)

const CUpti_Profiler_Host_GetMetricProperties_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_Profiler_Host_GetMetricProperties_Params,
                                                                                               metricCollectionScope)

const CUpti_Profiler_Host_GetRangeName_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_Profiler_Host_GetRangeName_Params,
                                                                                        pRangeName)

const CUpti_Profiler_Host_EvaluateToGpuValues_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_Profiler_Host_EvaluateToGpuValues_Params,
                                                                                               pMetricValues)

const CUpti_Profiler_Host_ConfigAddMetrics_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_Profiler_Host_ConfigAddMetrics_Params,
                                                                                            numMetrics)

const CUpti_Profiler_Host_GetConfigImageSize_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_Profiler_Host_GetConfigImageSize_Params,
                                                                                              configImageSize)

const CUpti_Profiler_Host_GetConfigImage_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_Profiler_Host_GetConfigImage_Params,
                                                                                          pConfigImage)

const CUpti_Profiler_Host_GetNumOfPasses_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_Profiler_Host_GetNumOfPasses_Params,
                                                                                          numOfPasses)

const CUpti_Profiler_Host_GetMaxNumHardwareMetricsPerPass_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_Profiler_Host_GetMaxNumHardwareMetricsPerPass_Params,
                                                                                                           maxMetricsPerPass)

const CUpti_Profiler_Host_GetSinglePassSets_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_Profiler_Host_GetSinglePassSets_Params,
                                                                                             ppSinglePassSets)

const CUpti_Profiler_Host_GetMetricsInSinglePassSet_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_Profiler_Host_GetMetricsInSinglePassSet_Params,
                                                                                                     pMetricsBuffer)

const CUpti_RangeProfiler_SetConfig_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_RangeProfiler_SetConfig_Params,
                                                                                     targetNestingLevel)

const CUpti_RangeProfiler_Enable_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_RangeProfiler_Enable_Params,
                                                                                  pRangeProfilerObject)

const CUpti_RangeProfiler_Disable_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_RangeProfiler_Disable_Params,
                                                                                   pRangeProfilerObject)

const CUpti_RangeProfiler_Start_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_RangeProfiler_Start_Params,
                                                                                 pRangeProfilerObject)

const CUpti_RangeProfiler_Stop_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_RangeProfiler_Stop_Params,
                                                                                isAllPassSubmitted)

const CUpti_RangeProfiler_PushRange_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_RangeProfiler_PushRange_Params,
                                                                                     pRangeName)

const CUpti_RangeProfiler_PopRange_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_RangeProfiler_PopRange_Params,
                                                                                    pRangeProfilerObject)

const CUpti_RangeProfiler_DecodeData_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_RangeProfiler_DecodeData_Params,
                                                                                      numOfRangeDropped)

const CUpti_RangeProfiler_GetCounterDataSize_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_RangeProfiler_GetCounterDataSize_Params,
                                                                                              counterDataSize)
...
const CUpti_RangeProfiler_GetCounterDataInfo_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_RangeProfiler_GetCounterDataInfo_Params,
                                                                                              numTotalRanges)

const CUpti_RangeProfiler_CounterData_GetRangeInfo_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_RangeProfiler_CounterData_GetRangeInfo_Params,
                                                                                                    rangeName)

const CUpti_PmSampling_SetConfig_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_PmSampling_SetConfig_Params,
                                                                                  hwBufferAppendMode)

const CUpti_PmSampling_Enable_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_PmSampling_Enable_Params,
                                                                               pPmSamplingObject)

const CUpti_PmSampling_Disable_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_PmSampling_Disable_Params,
                                                                                pPmSamplingObject)

const CUpti_PmSampling_Start_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_PmSampling_Start_Params,
                                                                              pPmSamplingObject)

const CUpti_PmSampling_Stop_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_PmSampling_Stop_Params,
                                                                             pPmSamplingObject)

const CUpti_PmSampling_DecodeData_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_PmSampling_DecodeData_Params,
                                                                                   overflow)

const CUpti_PmSampling_GetCounterAvailability_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_PmSampling_GetCounterAvailability_Params,
                                                                                               pCounterAvailabilityImage)

const CUpti_PmSampling_GetCounterDataSize_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_PmSampling_GetCounterDataSize_Params,
                                                                                           counterDataSize)

const CUpti_PmSampling_CounterDataImage_Initialize_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_PmSampling_CounterDataImage_Initialize_Params,
                                                                                                    pCounterData)

const CUpti_PmSampling_GetCounterDataInfo_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_PmSampling_GetCounterDataInfo_Params,
                                                                                           numCompletedSamples)

const CUpti_PmSampling_CounterData_GetSampleInfo_Params_STRUCT_SIZE = @CUPTI_PROFILER_STRUCT_SIZE(CUpti_PmSampling_CounterData_GetSampleInfo_Params,
                                                                                                  endTimestamp)
