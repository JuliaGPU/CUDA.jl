...
@testset "linker" begin
    ...
    if !Sys.iswindows()
        @test_throws_cuerror CUDAdrv.ERROR_UNKNOWN add_data!(link, "vadd_parent", UInt8[0])
    end
end

let
    link = CuLink()
    add_file!(link, joinpath(@__DIR__, "ptx/vadd_child.ptx"), CUDAdrv.CU_JIT_INPUT_PTX)
    ...
    options[CUDAdrv.CU_JIT_GENERATE_LINE_INFO] = true
