...
@testset "cudaconvert" begin
    ...
    @test cudaconvert(cuV) isa CuSparseDeviceVector{Float64, Cint, CUDA.AS.Global}

    A = sprand(10, 10, 0.5)
    cuA = CuSparseMatrixCSC(A)
    @test cudaconvert(cuA) isa CuSparseDeviceMatrixCSC{Float64, Cint, CUDA.AS.Global}

    cuA = CuSparseMatrixCSR(A)
    @test cudaconvert(cuA) isa CuSparseDeviceMatrixCSR{Float64, Cint, CUDA.AS.Global}

    cuA = CuSparseMatrixCOO(A)
    @test cudaconvert(cuA) isa CuSparseDeviceMatrixCOO{Float64, Cint, CUDA.AS.Global}

    cuA = CuSparseMatrixBSR(A, 2)
    @test cudaconvert(cuA) isa CuSparseDeviceMatrixBSR{Float64, Cint, CUDA.AS.Global}
end
