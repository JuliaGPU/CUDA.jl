...
@inline function call_batch_partition(vals::AbstractArray{T}, pivot, swap, b_sums, lo, hi,
    ...
    if threadIdx().x == 1
        ...
        # TODO: add wrappers
        device = Ref{Cint}()
        CUDACore.cudaGetDevice(device)
        max_blocks_y = Ref{Cint}()
        CUDACore.cudaDeviceGetAttribute(max_blocks_y, CUDACore.cudaDevAttrMaxGridDimY, device[])
...
function quicksort!(c::AbstractArray{T,N}; lt::F1, by::F2, dims::Int, partial_k=nothing,
    ...
    max_depth = CUDACore.limit(CUDACore.LIMIT_DEV_RUNTIME_SYNC_DEPTH) - 1
    len = size(c, dims)
