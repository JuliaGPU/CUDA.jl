...
@testset "completion callback failure" begin
    # Inject a callback-submission failure after a real launch in an isolated process.
    # Repeated calls need continued polling after the server accounts for the disarm.
    script = """
        ...
        out = CUDA.zeros(Int, 1)
        k = @cuda launch=false kernel(out)
        @eval CUDACore hostcall_disarm!(::CuStream) = error("callback submission failed")
        @test_throws "callback submission failed" k(out)
        synchronize(; blocking=true)
        @test Array(out) == [100]
        println("OK")
    """
    proc, out, err = julia_exec(`-e $script`)
    success(proc) || @error "callback recovery subprocess failed" stdout=out stderr=err
    @test success(proc)
    @test occursin("OK", out)
end
