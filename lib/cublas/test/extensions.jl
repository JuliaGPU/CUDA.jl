    ...
    @testset for elty in [Float32, Float64, ComplexF32, ComplexF64]
        ...
        for (opchar,opfun) in (('N',identity), ('T',transpose), ('C',adjoint))
            ...
            @testset "getrs_strided_batched!" begin
                ...
                d_A                 = CuArray(A)
                d_A2                = copy(d_A)
                d_pivot, info, d_LU = cuBLAS.getrf_strided_batched!(d_A, true)
                @test d_LU == d_A
                d_pivot2            = similar(d_pivot)
                info2               = similar(info)
                cuBLAS.getrf_strided_batched!(d_A2, d_pivot2, info2)
                @test isapprox(d_pivot, d_pivot2)
                @test isapprox(info, info2)
                B                   = rand(elty,n,m,k)
                d_B                 = CuArray(B)
                info2, d_Bhat       = cuBLAS.getrs_strided_batched!(opchar, d_LU, d_B, d_pivot)
                @test d_Bhat == d_B
                ...
                @test_throws DimensionMismatch cuBLAS.getrs_strided_batched!(opchar, d_A_bad, d_B, d_pivot)
